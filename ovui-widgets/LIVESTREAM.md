# OvGear Livestream — Bring-Up Guide (historical reproduction, 2026-05-03)

> **HISTORICAL DOCUMENT — NOT CURRENT SETUP GUIDANCE.** This entire guide,
> from this point through every command below, is a dated reproduction
> record of the livestream bring-up as validated on **2026-05-03** against
> the retired standalone `ovgear` repository and pinned feature branches.
> That repository topology has since merged into this monorepo (`ovgear` is
> now `ovui-widgets`), several of the pinned branches/commits no longer
> exist, and individual claims below (for example, ovrtx wheel availability)
> reflect that date, not the present. **Do not follow any command in this
> document as current setup.** For current usage, install per the
> *Installing from built wheels* section of `ovui-widgets/README.md`. The
> current livestream entry points are: the legacy **windowed viewport
> stream** — run the normal windowed app with `OVGEAR_LIVESTREAM=1` set —
> and the **full-UI headless export**, which requires **both**
> `OMNIUI_HEADLESS=1` **and** `OVGEAR_LIVESTREAM=1`
> (`python -m ovui_widgets.app.headless` sets the headless mode but does
> not stream by itself). Both paths need the external ovstream SDK at
> runtime. No current end-to-end livestream setup guide is provided; the
> launch commands are documented in
> `ovui-data-adapters/docs/kit-runtime.md` (§6, "Run the application").

> **Goal (historical):** Start from a git clone and reach a state where a
> browser displays OvGear's full UI live, mouse clicks select prims, the
> File menu opens, and keyboard input filters the Stage Browser — all over
> WebRTC.
...
At the time of this reproduction (2026-05-03), the ovrtx build used here was
installed editable from a local clone; the matching package was not then on
the public index. (This is no longer true: ovrtx wheels have since been
published on public PyPI — `pip install ovrtx` — but this historical recipe
was validated against the editable clone below.)
...
### 5.5 Install OvGear (the `ovui-widgets` distribution)
...
ssh -N -L 8080:localhost:8080 -L 49100:localhost:49100 <user>@<gpu-server>
```
