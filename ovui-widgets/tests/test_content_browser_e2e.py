...
def _author_loaded_sphere_radius(app: Application, value: float) -> None:
    stage = app._stage_adapter.stage
    stage.GetPrimAtPath("/Sphere").GetAttribute("radius").Set(value)


def _read_sphere_radius(path: str) -> float:
    from pxr import Usd

    stage = Usd.Stage.Open(path)
    return stage.GetPrimAtPath("/Sphere").GetAttribute("radius").Get()
...
class TestFullFlowOpenSave:
    ...
    def test_open_then_save_as_cycle(self, app, tmp_path):
        ...
        with patch(
            "ovui_widgets.content.file_importer.FileImporterHelper.instance",
        ) as MockInstance:
        ...
        assert _read_sphere_radius(save_path) == 2.75
    ...
    def test_save_direct_after_open_uses_same_path(self, app, tmp_path):
        ...
        assert app._current_file_path == seed_path
        _author_loaded_sphere_radius(app, 4.5)

        from ovui_widgets.app.menu_bar import _on_save_clicked
        _on_save_clicked(app)

        # Path unchanged; authored edit round-trips through the file on disk.
        assert os.path.exists(seed_path)
        assert app._current_file_path == seed_path
        assert _read_sphere_radius(seed_path) == 4.5
