...
"""CI invariant: importing ``ovui_widgets.stage`` must NOT pull in ``pxr``.

Step 26 (Rev 4 §10.5): analogous to
``test_ovui_widgets_viewport_no_usd_at_import.py``. The stage widget
...
into ``ovui_widgets/stage/__init__.py`` would force every
``import ovui_widgets.stage`` to load USD — exactly the regression the
...
def test_import_ovui_widgets_stage_does_not_load_pxr():
    """``import ovui_widgets.stage`` must NOT bring ``pxr`` into ``sys.modules``."""
    ...
    code = child_dll_preamble + textwrap.dedent(
        ...
        if leaked:
            sys.stderr.write(
                "ovui_widgets.stage import leaked pxr submodules: "
                + ",".join(leaked)
                + "\\n"
            )
            sys.exit(1)
