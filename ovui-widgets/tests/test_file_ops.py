...
from ovui_widgets.content.widget.file_ops import (
    _LOCAL_SCHEME,
...
from ovui_data_adapters.services.content.file_operations import (
    ContentFileRecord,
    _next_copy_name as _service_next_copy_name,
    duplicate_items as service_duplicate_items,
)
...
class TestNextCopyName:
    ...
    def test_widget_path_uses_service_policy(self):
        assert _next_copy_name is _service_next_copy_name
...
class TestDuplicateItemsAgainstMockBackend:
    ...
    def test_service_duplicate_accepts_neutral_records(self):
        backend = MockBackend()
        ...
        success, errors = service_duplicate_items(backend, [record])
        assert success == 1
        assert errors == []
        _, entries = backend.list_dir("mock://Home/Documents/Projects")
        assert "demo Copy.usda" in {entry.name for entry in entries}

    def test_service_file_operations_import_without_ui_runtime(self):
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    "import sys; "
                    "from ovui_data_adapters.services.content.file_operations "
                    "import ContentFileRecord, next_copy_name; "
                    "record = ContentFileRecord('/a.usd', 'a.usd', False); "
                    ...
                    "print(record, next_copy_name('a.usd', False, set()), forbidden); "
        ...
        assert proc.returncode == 0, (
            "service file operations imported UI/runtime modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "a Copy.usd" in proc.stdout
