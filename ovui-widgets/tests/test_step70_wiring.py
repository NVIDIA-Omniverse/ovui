...
class TestSnapSystemSettings:
    ...
    def test_grid_size_setting_updates_live_provider(self, app):
        app.settings.set("snap.grid_size", 0.25)

        assert app._grid_snap_provider.grid_size == pytest.approx(0.25)
        app._snap_system.enable(True)
        assert app._snap_system.snap([0.31, 0.62, 1.13]) == pytest.approx(
            [0.25, 0.5, 1.25]
        )

    def test_invalid_grid_size_setting_keeps_last_valid_value(self, app):
        app.settings.set("snap.grid_size", 0.5)

        app.settings.set("snap.grid_size", 0.0)

        assert app._grid_snap_provider.grid_size == pytest.approx(0.5)
