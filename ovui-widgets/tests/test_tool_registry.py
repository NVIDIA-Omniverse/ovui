...
"""Tests for :class:`ovui_widgets.viewport.manipulator_registry.ToolRegistry`.
...
def test_settings_subscription_inert_on_dead_generation():
    from types import SimpleNamespace

    import ovui_widgets.viewport.viewport_widget as viewport_mod
    from ovui_widgets.viewport.manipulator_registry import ToolRegistry
    ...
    settings = _Settings()
    changes = []
    dead = viewport_mod._Generation(None)  # never published
    registry = ToolRegistry(
        settings=settings,
        on_tool_changed=lambda old, new: changes.append((old, new)),
        generation=dead,
    )
    settings._fn("key", "rotate")
    assert changes == []
    assert registry.active_tool == "translate"
    # a published generation lets the subscription work again
    dead.alive = True
    settings._fn("key", "rotate")
    assert registry.active_tool == "rotate"
