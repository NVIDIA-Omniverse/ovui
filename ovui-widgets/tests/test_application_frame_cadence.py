...
def _release(app: Application) -> None:
    ...
    if viewport is not None and hasattr(viewport, "destroy"):
        viewport.destroy()
...
class TestPhysicsAlwaysAdvancesOnEveryTick:
    ...
    def test_adapter_physics_ticks_before_viewport_update_and_render(self):
        app = _fresh_app()
        events = []

        class _PhysicsControls:
            def tick(self, tick_dt):
                events.append(("physics.tick", tick_dt))

        class _Viewport(_FakeViewport):
            def update(self, tick_dt):
                events.append(("viewport.update", tick_dt))
                super().update(tick_dt)

            def render(self, render_dt):
                events.append(("viewport.render", render_dt))
                return super().render(render_dt)

        try:
            vp = _Viewport(render_returns=True)
            app._adapter_session = SimpleNamespace(
                physics_controls=_PhysicsControls(),
            )
            app._viewport_window = vp

            app._on_frame_update(0.005)

            assert events[0] == ("physics.tick", 0.005)
            assert events[1] == ("viewport.update", 0.005)
            assert events[2][0] == "viewport.render"
        finally:
            _release(app)
...
class TestStandaloneFramePacing:
    def test_standalone_loop_keeps_viewport_render_gate(self):
        app = _fresh_app()
        try:
            assert app._viewport_render_clock.target_fps == 120.0
            app._use_standalone_frame_pacing()
            assert app._viewport_render_clock.target_fps == 120.0
        finally:
            _release(app)
...
class TestRendererSwapResetsClock:
    def test_set_renderer_resets_clock_via_application(self):
        ...
        try:
            ...
            assert app._viewport_render_clock._next_due_time is None
        finally:
            _release(app)
