...
class TestViewportHasSceneViewLayer:
    ...
    def test_transform_manipulator_scene_uses_default_backend(self):
        ...
        assert isinstance(vp._scene_view, sc.SceneView)
        assert isinstance(vp._transform_manipulator, TransformManipulator)
    ...
    def test_registered_scene_view_factory_replaces_default_backend(self):
        renderer = MockRendererAdapter()
        vp = ViewportWidget(services=None, renderer=renderer)
        custom_scene_view = sc.SceneView()
        unregister = vp.register_manipulator_scene_view_factory(
            lambda: custom_scene_view,
            name="test-backend",
        )
        ...
        assert vp._scene_view is custom_scene_view
        assert state["backend"] == "test-backend"
        assert state["fallback_to_default_scene"] is False
        unregister()
        assert vp._manipulator_scene_view_factories == []
        vp.destroy()

    def test_failed_scene_view_factory_falls_back_to_default_backend(self):
        renderer = MockRendererAdapter()
        vp = ViewportWidget(services=None, renderer=renderer)
        ...
        vp.register_manipulator_scene_view_factory(fail, name="test-backend")
        vp._build_ui()
        state = vp.get_manipulator_scene_backend_state()

        assert isinstance(vp._scene_view, sc.SceneView)
        assert state["backend"] == "default"
        assert state["requested_backend"] == "test-backend"
        assert state["fallback_to_default_scene"] is True
        assert state["error"] == "optional backend failed"
        assert state["transform_manipulator_present"] is True
        vp.destroy()
