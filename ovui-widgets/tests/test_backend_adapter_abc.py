...
"""Tests for ovui_widgets.content.backends — BackendAdapter ABC and supporting types.
...
from ovui_widgets.content.backends import (
    BackendAdapter,
    BackendChangeEvent,
    BackendFileFlags,
    BackendListEntry,
...
OVUI_WIDGETS_PROJECT_ROOT = pathlib.Path(__file__).resolve().parents[1]
...
class TestImports:
    ...
    def test_package_all_is_complete(self):
        ...
        import ovui_widgets.content.backends as pkg
        assert {
            "BackendAdapter",
            "BackendChangeEvent",
            "BackendFileFlags",
    ...
    def test_ovui_widgets_exports_are_services_objects(self):
        from ovui_data_adapters.services.content.backends import (
            BackendAdapter as ServicesBackendAdapter,
            BackendChangeEvent as ServicesBackendChangeEvent,
            BackendFileFlags as ServicesBackendFileFlags,
            BackendListEntry as ServicesBackendListEntry,
            BackendResult as ServicesBackendResult,
        )

        assert BackendAdapter is ServicesBackendAdapter
        assert BackendChangeEvent is ServicesBackendChangeEvent
        assert BackendFileFlags is ServicesBackendFileFlags
        assert BackendListEntry is ServicesBackendListEntry
        assert BackendResult is ServicesBackendResult

    def test_ovui_widgets_backend_package_import_has_no_ui_runtime_dependency(self):
        env = dict(os.environ)
        existing_pythonpath = env.get("PYTHONPATH", "")
        entries = [str(OVUI_WIDGETS_PROJECT_ROOT)]
        ...
        proc = subprocess.run(
            [
                ...
                (
                    "import sys; "
                    "import ovui_widgets.content.backends as backends; "
                    "forbidden = [name for name in sys.modules "
                    "if name == 'omni.ui' or name.startswith('omni.ui.')]; "
                    "print(backends.BackendAdapter.__module__, forbidden); "
        ...
        assert proc.returncode == 0, (
            "ovui_widgets.content.backends must stay importable as a "
            "compatibility shim without loading omni.ui:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "ovui_data_adapters.services.content.backends.backend_adapter" in proc.stdout
...
class TestBackendFileFlags:
    ...
    def test_is_not_intflag(self):
        ...
        # ovui_widgets.app/adapters.py — avoid silent int coercion.
