...
from ovui_data_adapters.common import ChangeEvent, ChangeEventType, ReparentPosition

from ovui_widgets.common.testing.mock_stage import MockStageAdapter
from ovui_widgets.stage.drop_visual_controller import DropVisualController
from ovui_widgets.stage.hierarchy_model import DRAG_MIME, HierarchyItem, HierarchyModel
...
class TestHierarchyModelDragDrop:
    ...
    def test_get_drag_mime_data_returns_mime_constant(self):
        ...
        assert result == "application/ovui_widgets.stage-item"
    ...
    def test_drop_defers_reparent_until_native_callback_has_returned(
        self,
        monkeypatch,
    ):
        ...
        assert self.adapter._find_by_path("/World/Geometry/Sphere") is not None
        assert len(callbacks) == 1
        assert callbacks[0][0] == 0.0
        callbacks[0][1]()
        assert self.adapter._find_by_path("/World/Lights/Sphere") is not None

    def test_drop_re_resolves_adapter_items_after_intervening_resync(
        self,
        monkeypatch,
    ):
        from ovui_widgets.common import scheduler
        ...
        old_sphere = sphere.adapter_item
        geometry = self.adapter._find_by_path("/World/Geometry")
        assert geometry is not None
        new_sphere = type(old_sphere)(
            path=old_sphere.path,
            name=old_sphere.name,
            prim_type=old_sphere.prim_type,
            visible=old_sphere.visible,
            parent=geometry,
        )
        geometry.children[geometry.children.index(old_sphere)] = new_sphere

        old_lights = lights.adapter_item
        root = self.adapter.get_root()
        new_lights = type(old_lights)(
            path=old_lights.path,
            name=old_lights.name,
            prim_type=old_lights.prim_type,
            visible=old_lights.visible,
            children=list(old_lights.children),
            parent=root,
        )
        for child in new_lights.children:
            child.parent = new_lights
        root.children[root.children.index(old_lights)] = new_lights
        self.adapter._notify(
            ChangeEvent(
                changed_paths=(),
                resynced_paths=(new_sphere.path, new_lights.path),
        ...
        moved = self.adapter._find_by_path("/World/Lights/Sphere")
        assert moved is new_sphere
        assert moved.parent is new_lights

    def test_drop_aborts_when_source_is_removed_before_deferred_callback(
        self,
        monkeypatch,
    ):
        from ovui_widgets.common import scheduler
        ...
        self.model.drop(lights, sphere)
        self.adapter.remove("/World/Geometry/Sphere")
        callbacks.pop(0)[1]()

        assert self.adapter._find_by_path("/World/Geometry/Sphere") is None
        assert self.adapter._find_by_path("/World/Lights/Sphere") is None

    def test_drop_restores_remapped_selection_after_reparent_notices(
        self,
        monkeypatch,
    ):
        from ovui_widgets.common import scheduler
        ...
        sphere = self._item("/World/Geometry/Sphere")
        lights = self._item("/World/Lights")
        self.model._selected_items = [sphere]
        self.model.set_namespace_selection_remap_callback(remapped.append)
        ...
        assert len(callbacks) == 1
        callbacks.pop(0)[1]()
        assert remapped == [["/World/Lights/Sphere"]]
...
class TestUndoGroupWiring:
    ...
    def test_drop_ends_undo_group_and_reports_when_scheduled_reparent_raises(
        self,
        monkeypatch,
    ):
        from ovui_widgets.common import scheduler
        ...
        monkeypatch.setattr(
            ErrorReporter,
            "show_error",
            lambda message, duration_ms=5000: errors.append(message),
        )
        self.adapter.begin_undo_group = lambda label: order.append("begin")
        self.adapter.end_undo_group = lambda: order.append("end")

        def rejected_reparent(items, parent, position):
            order.append("reparent")
            raise ValueError("reparent rejected")

        self.adapter.reparent = rejected_reparent
        sphere = self._item("/World/Geometry/Sphere")
        lights = self._item("/World/Lights")
        ...
        assert order == ["begin", "reparent", "end"]
        assert errors == ["Cannot reparent prim: ValueError: reparent rejected"]
...
class TestApplicationOnDropMultiUrl:
    ...
    def test_multi_url_mixed_opens_first_usd_only(self, headless_app):
        ...
        with patch("ovui_widgets.common.error_reporter.ErrorReporter.show_status") as mock_show:
