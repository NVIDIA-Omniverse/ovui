...
"""Tests for ovui_widgets.common.adapters — all adapter ABCs and supporting types."""
...
class TestImports:
    ...
    def test_render_target_catalog_importable(self):
        assert RenderTargetCatalog is not None

    def test_render_target_activation_result_importable(self):
        assert RenderTargetActivationResult is not None

    def test_package_all_includes_stage_choice(self):
        import ovui_data_adapters.common as pkg
        assert "StageChoice" in pkg.__all__

    def test_package_all_includes_render_target_contracts(self):
        import ovui_data_adapters.common as pkg
        assert "RenderTargetCatalog" in pkg.__all__
        assert "RenderTargetActivationResult" in pkg.__all__
...
class TestStageAdapterMethods:
    ...
    @pytest.mark.parametrize(
        ...
        [
            "list_cameras",
            "read_camera_pose",
            "list_render_products",
            "get_render_target_catalog",
        ],
    )
...
class TestRendererAdapterMethods:
    ...
    def test_render_target_activation_default_rejects(self):
        renderer = _ConcreteRenderer()
        result = renderer.activate_render_target(
            target_id="target",
            render_product_path="/Render/Beauty",
        )
        assert result == RenderTargetActivationResult.rejected_result(
