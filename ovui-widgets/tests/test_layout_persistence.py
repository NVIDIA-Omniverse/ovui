...
class TestSaveLayout:
    ...
    def test_save_layout_stores_window_data_in_settings(self, app):
        ...
        with patch("ovui_widgets.app.layout._collect_layout", return_value=fake_data):
            with patch("ovui_widgets.app.layout.save_layout_data"):
                app._save_layout()
    ...
    def test_save_layout_empty_collect_does_not_update_settings(self, app):
        with patch("ovui_widgets.app.layout._collect_layout", return_value={}):
            app._save_layout()
    ...
    def test_save_layout_calls_save_layout_data_file(self, app):
        with patch("ovui_widgets.app.layout._collect_layout", return_value={"win": {}}):
            with patch("ovui_widgets.app.layout.save_layout_data") as mock_save:
                app._save_layout()
        mock_save.assert_called_once()

    def test_save_layout_does_not_raise_on_file_exception(self, app):
        with patch("ovui_widgets.app.layout._collect_layout", return_value={"win": {}}):
            with patch("ovui_widgets.app.layout.save_layout_data", side_effect=IOError("disk full")):
...
class TestRestoreLayout:
    ...
    def test_restore_uses_settings_data_when_present(self, app):
        ...
        with patch("ovui_widgets.app.layout._restore_layout") as mock_apply:
            app._restore_layout()
    ...
    def test_restore_skips_file_when_settings_has_data(self, app):
        fake_data = {"Stage Browser": {"visible": True}}
        app._settings.set(Application.LAYOUT_SETTINGS_KEY, fake_data)
        with patch("ovui_widgets.app.layout._restore_layout"):
            with patch("ovui_widgets.app.layout.load_layout") as mock_load:
                app._restore_layout()
    ...
    def test_restore_falls_back_to_default_when_no_file(self, app):
        with patch("os.path.exists", return_value=False):
            with patch("ovui_widgets.app.layout.apply_default_layout") as mock_default:
                app._restore_layout()
        mock_default.assert_called_once()

    def test_restore_falls_back_to_default_on_file_error(self, app):
        with patch("os.path.exists", return_value=True):
            with patch("ovui_widgets.app.layout.load_layout", side_effect=ValueError("bad json")):
                with patch("ovui_widgets.app.layout.apply_default_layout") as mock_default:
                    app._restore_layout()
        mock_default.assert_called_once()
    ...
    def test_restore_uses_default_save_path_when_unset(self, app):
        ...
        with patch("os.path.exists", return_value=False) as mock_exists:
            with patch("ovui_widgets.app.layout.apply_default_layout"):
                app._restore_layout()
        # os.path.exists should have been called with the expanded path
        assert call(expanded) in mock_exists.call_args_list
