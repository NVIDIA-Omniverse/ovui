...
"""Tests for undo / command / history behavior and ovui-widgets compatibility."""
...
import pytest
from ovui_data_adapters.services import undo as services_undo
from ovui_data_adapters.services.undo import Subscription

from ovui_widgets.common import undo as widgets_undo
from ovui_widgets.common.undo import Command, CommandCancelled, UndoGroup, UndoManager
...
class TestImportSurface:
    def test_ovui_widgets_exports_are_services_objects(self):
        assert widgets_undo.Command is services_undo.Command
        assert widgets_undo.CommandCancelled is services_undo.CommandCancelled
        assert widgets_undo.UndoGroup is services_undo.UndoGroup
        assert widgets_undo.UndoManager is services_undo.UndoManager
        assert widgets_undo.UndoManagerProtocol is services_undo.UndoManagerProtocol

    def test_command_cancelled_is_not_split_from_undo_manager(self):
        assert CommandCancelled is services_undo.CommandCancelled
        assert UndoManager is services_undo.UndoManager
    ...
    def test_services_import_has_no_ui_or_ovui_widgets_dependency(self):
        proc = subprocess.run(
            [
                ...
                (
                    "import sys; "
                    "from ovui_data_adapters.services.undo "
                    "import CommandCancelled, UndoManager; "
                    ...
                    "print(UndoManager.null().can_undo(), CommandCancelled.__name__, forbidden); "
        ...
        assert proc.returncode == 0, (
            "services undo must import without ovui-widgets or omni modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "False CommandCancelled []" in proc.stdout
...
class TestUndoGroup:
    ...
    def test_do_compensates_completed_commands_on_partial_failure(self):
        state = {"first": False}
        log = []
        ...
        class First(Command):
            ...
            def undo(self):
                log.append("first.undo")
                state["first"] = False

        class FailingSecond(Command):
            def do(self):
                log.append("second.do")
                raise RuntimeError("do failed")

            def undo(self):
                raise AssertionError("an incomplete command must not be compensated")

        group = UndoGroup("atomic do", [First(), FailingSecond()])

        with pytest.raises(RuntimeError, match="do failed"):
            group.do()

        assert state == {"first": False}
        assert log == ["first.do", "second.do", "first.undo"]

    def test_undo_reapplies_completed_commands_on_partial_failure(self):
        state = {"first": True, "second": True}
        log = []

        class FailingFirst(Command):
            def do(self):
                state["first"] = True
        ...
        class Second(Command):
            def do(self):
                raise AssertionError("undo compensation must use redo")

            def undo(self):
                log.append("second.undo")
                state["second"] = False

            def redo(self):
                log.append("second.redo")
                state["second"] = True

        group = UndoGroup("atomic undo", [FailingFirst(), Second()])

        with pytest.raises(RuntimeError, match="undo failed"):
            group.undo()

        assert state == {"first": True, "second": True}
        assert log == ["second.undo", "first.undo", "second.redo"]
...
class TestBasicPushUndoRedo:
    ...
    def test_failed_undo_retains_undo_entry(self):
        class FailsFirstUndo(CountingCommand):
            def undo(self):
                self.undo_count += 1
        ...
        assert mgr._undo_stack == [cmd]
        assert mgr._redo_stack == []
        assert mgr.undo() is True
        assert mgr._undo_stack == []
        assert mgr._redo_stack == [cmd]

    def test_failed_redo_retains_redo_entry(self):
        class FailsFirstRedo(CountingCommand):
            def redo(self):
                self.redo_count += 1
...
class TestGroups:
    ...
    def test_failed_cancel_group_restores_state_and_retains_group(self):
        state = {"first": False, "second": False}

        class FailsFirstUndo(SetValueCommand):
            def __init__(self):
                super().__init__(state, "first", True, False)
                self.undo_count = 0
        ...
        first = FailsFirstUndo()
        second = SetValueCommand(state, "second", True, False)
        mgr = UndoManager()
        mgr.begin_group("g")
        mgr.push(first)
        mgr.push(second)
        ...
        assert state == {"first": True, "second": True}
        assert mgr._group_stack == [("g", [first, second])]

        mgr.cancel_group()
        assert state == {"first": False, "second": False}
        assert mgr._group_stack == []
