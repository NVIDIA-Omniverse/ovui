...
``ovui_widgets.common.error_reporter``) would re-introduce the cyclic
...
openusd modules, just parses them. Two scans:
  ...
  2. Dynamic-import AST scan (Rev 3 — replaces Rev 2's broad string
     ...
     widget-side symbols (``ovui_widgets.viewport.camera_controller`` in
...
def test_openusd_dynamic_imports_do_not_target_widgets(py: pathlib.Path):
    """Reject ``__import__("ovui_widgets.…")`` / ``importlib.import_module("ovui_widgets.…")``.
