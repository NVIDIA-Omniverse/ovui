...
from ovui_data_adapters.services.content.navigation import (
    RecentFileList as ServiceRecentFileList,
)
from ovui_widgets.common.recent_files import RecentFileList


class TestRecentFileListServiceBoundary:
    def test_ovui_widgets_recent_file_list_keeps_service_behavior(self):
        assert issubclass(RecentFileList, ServiceRecentFileList)

    def test_service_recent_file_list_has_no_app_singleton_policy(self):
        assert not hasattr(ServiceRecentFileList, "instance")
        assert not hasattr(ServiceRecentFileList, "set_instance")

    def test_service_recent_file_list_imports_without_ui_runtime(self):
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    ...
                    "import RecentFileList; "
                    "recent = RecentFileList(['/a.usd']); "
                    "recent.add('/b.usd'); "
        ...
        assert proc.returncode == 0, (
            "service RecentFileList imported UI/runtime modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "['/b.usd', '/a.usd']" in proc.stdout
