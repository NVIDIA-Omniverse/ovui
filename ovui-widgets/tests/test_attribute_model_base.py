...
class TestBaseExceptionFinalization:
    """Round-25 F5: a buffered write that raises any BaseException must
    cancel the adapter transaction (never strand it) while the exact
    primary throwable propagates."""

    class _CancelRecordingAdapter(_FakeAdapter):
        ...
        def cancel_edit(self, attr_name: str) -> None:
            self.calls.append(("cancel_edit", attr_name))

    def _run(self, exc: BaseException) -> tuple:
        adapter = self._CancelRecordingAdapter(exc)
        model = AttributeModelBase(adapter, "radius", _metadata())
        model.begin_edit()
        model._value = 5.0   # buffered value differing from _prev_value
        primary = None
        try:
            model.end_edit()
        except BaseException as caught:  # noqa: BLE001 — asserted below
            primary = caught
        return adapter, model, primary

    def test_keyboard_interrupt_cancels_and_preserves_primary(self):
        exc = KeyboardInterrupt("hostile buffered write")
        adapter, model, primary = self._run(exc)
        assert primary is exc
        assert model.editing is False
        assert ("cancel_edit", "radius") in adapter.calls
        assert ("end_edit", "radius") not in adapter.calls

    def test_system_exit_and_runtime_error_also_cancel(self):
        for exc in (SystemExit("se"), RuntimeError("re")):
            adapter, model, primary = self._run(exc)
            assert primary is exc
            assert ("cancel_edit", "radius") in adapter.calls

    def test_cancel_failure_noted_primary_kept(self):
        exc = KeyboardInterrupt("primary")

        class _HostileCancel(self._CancelRecordingAdapter):
            def cancel_edit(self, attr_name: str) -> None:
                raise ValueError("cancel also failed")

        adapter = _HostileCancel(exc)
        model = AttributeModelBase(adapter, "radius", _metadata())
        model.begin_edit()
        model._value = 5.0
        try:
            ...
            assert any("cancel" in note for note in notes)
        else:
            raise AssertionError("primary did not propagate")

    def test_model_cancel_edit_releases_adapter_transaction(self):
        adapter = _FakeAdapter()
        adapter.cancel_edit = lambda name: adapter.calls.append(
            ("cancel_edit", name))
        model = AttributeModelBase(adapter, "radius", _metadata())
        model.begin_edit()
        model.cancel_edit()
