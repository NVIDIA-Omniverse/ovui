...
from ovui_widgets.content.widget.file_browser_widget import FileBrowserWidget
...
class TestSurface:
    def test_import_from_window_subpackage(self):
        from ovui_widgets.content.window import ContentBrowserWindow as CW

        assert CW is ContentBrowserWindow

    def test_import_from_package(self):
        from ovui_widgets.content import ContentBrowserWindow as CW

        assert CW is ContentBrowserWindow

    def test_package_all_contains_window(self):
        import ovui_widgets.content as pkg

        assert "ContentBrowserWindow" in pkg.__all__

    def test_window_subpackage_all_contains_window(self):
        import ovui_widgets.content.window as pkg
...
class TestDefaultStartUrl:
    def test_default_start_url_is_home(self):
        ...
        # normalised ``file://`` URL. Decode at the URL/filesystem
        # boundary before comparing filesystem identity.
        home = os.path.expanduser("~")
        w = ContentBrowserWindow()
        assert w._start_url.startswith("file://")
        assert Path(_url_to_fspath(w._start_url)) == Path(home)
        w.destroy()
