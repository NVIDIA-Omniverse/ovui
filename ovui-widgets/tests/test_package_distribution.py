...
"""Tests for Issue #31 Step 2 — ovui-widgets packaging distribution metadata.

Verifies that:
- The installed distribution is named ``ovui-widgets-app``
...
def dist() -> importlib.metadata.Distribution:
    """Return the installed ovui-widgets-app distribution metadata."""
    return importlib.metadata.distribution("ovui-widgets-app")


# ── Distribution metadata (via importlib.metadata) ────────────────────────────

class TestDistributionMetadata:
    """Verify the installed ovui-widgets-app distribution via importlib.metadata."""

    def test_distribution_name(self, dist):
        assert dist.metadata["Name"] == "ovui-widgets-app"
...
class TestConsoleScripts:
    """Verify all four console scripts are registered for ovui_widgets."""

    @pytest.fixture(scope="class")
    def script_names(self, dist):
        """Set of console-script names registered by ovui_widgets."""
        return {ep.name for ep in dist.entry_points if ep.group == "console_scripts"}

    def test_ovui_widgets_registered(self, script_names):
        assert "ovui-widgets" in script_names
    ...
    def test_ovui_widgets_headless_registered(self, script_names):
        assert "ovui-widgets-headless" in script_names
    ...
    def test_ovui_widgets_points_to_main_sync(self, dist):
        ep = next(
            ep for ep in dist.entry_points
            if ep.group == "console_scripts" and ep.name == "ovui-widgets"
        )
        assert ep.value == "ovui_widgets.app.__main__:main_sync"
    ...
    def test_ovui_widgets_headless_points_to_headless_main(self, dist):
        ep = next(
            ep for ep in dist.entry_points
            if ep.group == "console_scripts" and ep.name == "ovui-widgets-headless"
        )
        assert ep.value == "ovui_widgets.app.headless:main"
...
class TestPackageData:
    ...
    def test_style_icons_directory_exists(self):
        # Step 8/13: icon directories moved from ``ovui_widgets/app/style/icons``
        # to ``ovui_widgets/common/style/icons``; package-data ownership
        # transferred to ``dist/common/pyproject.toml``.
        icons_dir = Path(__file__).parent.parent / "ovui_widgets" / "common" / "style" / "icons"
...
class TestEntryPoint:
    def test_entry_point_module_importable(self):
        mod = importlib.import_module("ovui_widgets.app.__main__")
...
class TestMainModule:
    ...
    def test_module_docstring_or_file_comment(self):
        ...
        path = Path(__file__).parent.parent / "ovui_widgets" / "app" / "__main__.py"
        assert path.exists()
    ...
    def test_main_sync_target_in_file(self):
        # The canonical definition lives in ovui_widgets.app/__main__.py (Step 3 rename).
        # The ovui_widgets.app/__main__.py shim re-exports it.
        path = Path(__file__).parent.parent / "ovui_widgets" / "app" / "__main__.py"
        content = path.read_text(encoding="utf-8")
...
# ── Windows launcher (scripts/run_ovui_widgets_windows.py) ──────────────────────

class TestWindowsLauncher:
    """Verify the Windows bootstrap launcher dispatches to ovui_widgets.app, not ovgear."""

    @pytest.fixture(scope="class")
    def launcher_source(self) -> str:
        path = Path(__file__).parent.parent / "scripts" / "run_ovui_widgets_windows.py"
        assert path.exists(), "scripts/run_ovui_widgets_windows.py must exist"
    ...
    def test_dispatches_to_ovui_widgets_app(self, launcher_source):
        assert 'run_module("ovui_widgets.app"' in launcher_source

    def test_does_not_dispatch_to_deleted_ovgear(self, launcher_source):
        assert 'run_module("ovgear"' not in launcher_source

    def test_references_ovui_widgets_app_in_docstring(self, launcher_source):
        assert "ovui_widgets.app.__main__" in launcher_source
    ...
    def test_enables_zero_copy_by_default(self, launcher_source):
        assert 'setdefault("OVGEAR_ZERO_COPY", "1")' in launcher_source


# ── Windows BAT launcher (scripts/ovui-widgets-win.bat) ─────────────────────────

class TestWindowsBatFiles:
    """Verify the Windows .bat launcher structure after the ovui-widgets rename."""

    @pytest.fixture(scope="class")
    def ovui_widgets_bat(self) -> str:
        path = Path(__file__).parent.parent / "scripts" / "ovui-widgets-win.bat"
        assert path.exists(), "scripts/ovui-widgets-win.bat must exist"
        return path.read_text(encoding="utf-8")

    def test_ovui_widgets_bat_exists(self):
        path = Path(__file__).parent.parent / "scripts" / "ovui-widgets-win.bat"
        assert path.exists()

    def test_ovui_widgets_bat_invokes_python_bootstrap(self, ovui_widgets_bat):
        assert "run_ovui_widgets_windows.py" in ovui_widgets_bat
        assert "run_ovgear_windows.py" not in ovui_widgets_bat

    def test_ovui_widgets_bat_sets_usd_install_root(self, ovui_widgets_bat):
        assert "USD_INSTALL_ROOT" in ovui_widgets_bat

    def test_ovui_widgets_bat_sets_skip_usd_check(self, ovui_widgets_bat):
        assert "OVRTX_SKIP_USD_CHECK" in ovui_widgets_bat
