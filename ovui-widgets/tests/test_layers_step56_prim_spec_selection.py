...
tree-view selection is mirrored to :class:`~ovui_widgets.common.selection.SelectionBus`
under :data:`~ovui_widgets.layers.selection_watch.LAYERS_SELECT_SOURCE` so Stage
...
- :data:`LAYERS_SELECT_SOURCE` is the namespaced string the plan
  pins (``"ovui_widgets.layers:select"``).
- Re-export from :mod:`ovui_widgets.layers` — call sites import it by name.
"""
...
class TestSourceConstant:
    def test_value_is_namespaced(self) -> None:
        """Plan pins the source string to ``"ovui_widgets.layers:select"`` —
        ...
        assert LAYERS_SELECT_SOURCE == "ovui_widgets.layers:select"

    def test_reexported_from_package(self) -> None:
        """Call sites import the constant from :mod:`ovui_widgets.layers` —
        the sub-module path stays stable but the package surface is
        the canonical import site."""
        import ovui_widgets.layers
        from ovui_widgets.layers.selection_watch import LAYERS_SELECT_SOURCE as module_level

        assert ovui_widgets.layers.LAYERS_SELECT_SOURCE is module_level
