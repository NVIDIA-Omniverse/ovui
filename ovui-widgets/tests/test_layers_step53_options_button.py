...
Covers the LAYERS-PLAN Step 53 deliverables:
  ...
  * Toggle through :meth:`OptionsButton.toggle` flips the backing
    ...
    the :class:`ovui_widgets.common.settings.Settings` store (persistence).
...
from ovui_widgets.layers.options_button import MENU_ITEMS
...
def fake_ui_module() -> Any:
    """Patch :mod:`ovui_widgets.layers.options_button`'s ``ui`` to a fake recorder.

    Yields the fake module so tests can inspect the last
    :class:`_FakeMenu` via ``fake.menus[-1]``.
    """
    import ovui_widgets.layers.options_button as mod
