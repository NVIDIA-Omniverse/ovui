...
from ovui_data_adapters.services import selection as services_selection
from ovui_widgets.common import selection as widgets_selection
...
class TestImportSurface:
    def test_ovui_widgets_selection_exports_services_objects(self):
        assert widgets_selection.SelectionBus is services_selection.SelectionBus
        assert widgets_selection.SelectionBusError is services_selection.SelectionBusError
        assert (
            widgets_selection.SelectionChangedEvent
            is services_selection.SelectionChangedEvent
        )
        assert widgets_selection.SelectionItem is services_selection.SelectionItem
        assert widgets_selection.SelectionSnapshot is services_selection.SelectionSnapshot
        assert widgets_selection.Subscription is services_selection.Subscription

    def test_services_selection_imports_without_ui_runtime_modules(self):
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    ...
                    "SelectionBus, SelectionBusError; "
                    ...
                    "print(SelectionBus().get_snapshot().paths(), "
                    "SelectionBusError.__name__, forbidden); "
        ...
        assert proc.returncode == 0, (
            "selection service imported UI/runtime modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "[] SelectionBusError []" in proc.stdout
