...
class TestStatusBarWidget:
    ...
    def test_status_frame_stays_visible_to_preserve_viewport_size(self):
        bar, frame = self._make_bar()

        assert frame.visible is True
        bar.show_message("Created /World/Cube.", level="success")
        assert frame.visible is True
        bar.clear()
        assert frame.visible is True
...
class TestShowStatusWithLabel:
    ...
    def test_prefers_status_bar_over_raw_label_when_both_are_available(self):
        mock_bar = MockStatusBar()
        ErrorReporter._set_status_bar(mock_bar)
        ErrorReporter.show_status("msg")
        assert self.label.text == ""
        assert mock_bar.calls == [("msg", 3000, "")]
