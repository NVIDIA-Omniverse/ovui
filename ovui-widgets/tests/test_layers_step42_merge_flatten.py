...
- **Adapter snapshot/restore API** —
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.snapshot_layer`,
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.restore_layer_from_snapshot`,
  and :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.transfer_layer_content`
  on the ABC + mock + USD adapters.
- **Two undoable commands** —
  :class:`~ovui_widgets.layers.commands.MergeDownCommand` and
  :class:`~ovui_widgets.layers.commands.FlattenSublayersCommand`. Both snapshot
  every layer they touch before mutating, so ``undo`` rebuilds the
  pre-merge state byte-for-byte.
- **Two confirmation dialogs** — :func:`ovui_widgets.common.dialogs.confirm_merge_down_dialog`
  and :func:`ovui_widgets.common.dialogs.confirm_flatten_dialog`. Both are thin
  wrappers around :func:`ovui_widgets.common.dialogs.confirm_dialog` so the
...
- **Two context-menu entries** — "Merge Down" gated on
  :func:`~ovui_widgets.layers.context_menu.has_sibling_below`, "Flatten
  Sublayers" gated on :func:`~ovui_widgets.layers.context_menu.has_sublayers`.
  Both land in :data:`~ovui_widgets.layers.context_menu.GROUP_DESTRUCTIVE` and
...
class TestMergeDownEntry:
    ...
    def test_click_opens_confirm_dialog(
        ...
        with patch(
            "ovui_widgets.common.dialogs.confirm_merge_down_dialog",
            _fake_dialog,
        ):
...
class TestFlattenEntry:
    ...
    def test_click_opens_confirm_dialog_with_count(
        ...
        with patch(
            "ovui_widgets.common.dialogs.confirm_flatten_dialog",
            _fake_dialog,
        ):
...
class TestUsdAdapterMergeFlatten:
    def _build(self, tmp_path):
        ...
        adapter.get_sublayer_identifiers(adapter.get_root_layer())
        return adapter, top.identifier, middle.identifier
