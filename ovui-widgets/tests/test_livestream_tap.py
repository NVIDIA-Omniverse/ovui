...
def _channel_last_mapping(width, height, ptr=0x123456):
    return types.SimpleNamespace(
        data=ptr,
        shape=(height, width, 4),
        dtype=types.SimpleNamespace(lanes=1, bits=8),
    )
...
def test_tee_to_ovstream_accepts_current_channel_last_ldr_layout(monkeypatch):
    """Current OVRTX exposes LdrColor as scalar-lane ``(H, W, 4)``.

    The provider-neutral tap must stream that layout just like the legacy
    ``(H, W)`` + four-lane DLPack representation.
    """
    _ovstream, server = _install_mock_ovstream(monkeypatch, connected=True)
    events = []
    cudart = _make_cudart_mock(events, ptrs=iter([0xAA, 0xBB]))
    ...
    pushed = tap.tee_to_ovstream(
        _channel_last_mapping(3, 2, ptr=0x1111),
        3,
        2,
    )

    assert pushed is True
    assert ("d2d", 0xAA, 0x1111, 3 * 2 * 4) in events
    frame = server.stream_video.call_args.args[0]
    assert (frame.width, frame.height, frame.pitch_bytes) == (3, 2, 12)
