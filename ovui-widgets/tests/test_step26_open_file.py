...
def app():
    a = Application()
    # These are headless stage/bootstrap tests: no viewport is constructed, so
    # pre-creating a native OVRTX renderer cannot contribute to the assertion
    # and can abort a generic CPU test environment. ``None`` records the same
    # completed-but-unavailable bootstrap outcome used by the application.
    a._startup_prebuilt_renderer = None
    yield a
    a.shutdown()
