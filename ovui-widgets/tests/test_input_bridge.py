...
"""Tests for ovui_widgets.app._input_bridge.RemoteInputBridge (issue #34, Step 3.2).
...
def test_clamp_and_one_time_warn(caplog: pytest.LogCaptureFixture) -> None:
    ...
    caplog.set_level(logging.WARNING, logger="ovui_widgets.app.input_bridge")
...
def test_move_preserves_latest_modifier_mask() -> None:
    bridge = RemoteInputBridge(width=_W, height=_H)
    bridge.on_mouse_event(_move(400, 300, mods=_NVST_MF_ALT))
    xy, events = bridge.drain()
    assert xy == (400, 300)
    assert events == []
    assert bridge.mouse_modifier_mask() == _NVST_MF_ALT


def test_mouse_modifier_state_reports_changed_zero_mask() -> None:
    bridge = RemoteInputBridge(width=_W, height=_H)
    bridge.on_mouse_event(_move(400, 300, mods=_NVST_MF_ALT))
    assert bridge.consume_mouse_modifier_state() == (_NVST_MF_ALT, True)
    assert bridge.consume_mouse_modifier_state() == (_NVST_MF_ALT, False)

    bridge.on_mouse_event(_move(410, 300, mods=0))
    assert bridge.consume_mouse_modifier_state() == (0, True)
    assert bridge.consume_mouse_modifier_state() == (0, False)
...
def test_consecutive_modern_wheel_fields_accumulate() -> None:
    """Browser/WebRTC wheel packets carry float deltas in scroll_x/y."""
    bridge = RemoteInputBridge(width=_W, height=_H)
    bridge.on_mouse_event(_wheel(0, 0, scroll_y=0.5))
    bridge.on_mouse_event(_wheel(0, 0, scroll_y=1.25))
    _xy, events = bridge.drain()
    assert len(events) == 1
    only = events[0]
    assert isinstance(only, MouseEvent) and only.type == MouseEventType.WHEEL
    assert only.data == 0
    assert only.data2 == 0
    assert only.scroll_y == pytest.approx(1.75)
