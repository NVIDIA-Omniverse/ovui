...
Encodes the Phase 0 reproduction as an automated test:
    ...
    Launch real ``python -m ovui_widgets.app cube.usda`` as a subprocess
...
* Round 1 F11 / Round 2 F8 — invoke the LITERAL command
  ``[sys.executable, "-m", "ovui_widgets.app", scene]``. No ``runpy.run_module``
...
* Round 1 F12 — subprocess timeout = hard test failure. A hung shutdown
  is a regression, not a silent skip, and the timeout path must tear down
  the child process group before failing.
...
* Round 6 F5 — also prepend the repo root so ``python -m ovui_widgets.app``
  can resolve the ``ovui_widgets.app`` package.
"""
...
# ``python -m ovui_widgets.app`` in our isolated cwd, sys.path[0] is the cwd
...
SITECUSTOMIZE = textwrap.dedent('''
    ...
    # Defer the install until after ovui_widgets.app.application is importable.
    # We schedule it as a sys.audit hook so it fires the first time
    # anyone imports ovui_widgets.app.application — i.e. inside ovui_widgets.app/__main__.main().
    import sys as _sys
    ...
    def _audit(event, args):
        ...
        if event == "import":
            ...
            if mod == "ovui_widgets.app.application":
...
def _child_env(headless: bool, tmp_home: Path) -> dict:
    ...
    #   2. REPO       — for ``python -m ovui_widgets.app`` to find the package
    #
    ...
    # subprocess can hang during headless renderer startup. When the scrubbed
    # child has no usable OpenUSD runtime/provider, the test skips explicitly
    # instead of failing noisily during provider discovery.
    pp = (
        f"{tmp_home}:{REPO}:"
...
def _run_ovgear_subprocess(headless: bool, tmp_path: Path):
    ...
    Raises ``pytest.fail`` on subprocess timeout after killing the child
    process group (Round 1 F12).
    """
    ...
    # Round 2 F8: literal `python -m ovui_widgets.app` — same as a real user.
    cmd = [sys.executable, "-m", "ovui_widgets.app", str(SCENE)]
    ...
    try:
        out, err = proc.communicate(timeout=120)  # Round 1 F12: hard fail on hang.
        ...
        try:
            ...
            out = second_timeout.stdout or out
            err = second_timeout.stderr or err
            try:
                proc.kill()
            except OSError:
                pass
        ...
        pytest.fail(
            ...
            f"stdout tail:\n"
            f"{out.decode(errors='replace')[-1500:]}\n"
            f"stderr tail:\n"
