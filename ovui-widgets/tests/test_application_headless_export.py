...
The hook fires once per frame after `await ui.next_frame()`, once ovui has
rendered the prepared frame. Each tick must:
...
def test_headless_export_uses_exact_supported_headless_predicate(
    monkeypatch,
    headless_value,
    expected,
):
    ...
    assert Application._headless_export_env_active() is expected


def test_setup_skipped_when_only_omniui_headless_set(monkeypatch, headless_app):
    """Headless ovui_widgets.app without livestream still must not start the
