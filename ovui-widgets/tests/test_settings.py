...
"""Tests for ovui_widgets.common.settings — Settings class and Subscription."""
...
from ovui_data_adapters.services.settings import (
...
class TestServiceSettingsSplit:
    def test_service_settings_has_no_ovui_widgets_defaults(self):
        s = ServiceSettings()
        assert s.get("ui.theme") is None
        assert s.get("layout.save_path") is None
        assert s.get("viewport.camera.fov") is None

    def test_service_settings_accepts_injected_defaults(self):
        s = ServiceSettings({"ui.theme": "neutral", "items": []})
        assert s.get("ui.theme") == "neutral"
        s.get("items").append("a")
        assert ServiceSettings({"items": []}).get("items") == []

    def test_ovui_widgets_settings_subclasses_service_store(self):
        assert issubclass(Settings, ServiceSettings)
        assert Subscription is ServiceSubscription
    ...
    def test_service_import_without_ui_runtime_modules(self):
        ...
        proc = subprocess.run(
            [
                ...
                (
                    "import sys; "
                    "from ovui_data_adapters.services.settings "
                    "import Settings, Subscription; "
                    "s = Settings({'x': 1}); "
                    "seen = []; "
                    "sub = s.subscribe('x', lambda k, v: seen.append((k, v))); "
                    "s.set('x', 2); "
                    ...
                    "print(s.get('x'), seen, Subscription.__name__, forbidden); "
        ...
        assert "2 [('x', 2)] Subscription []" in proc.stdout
