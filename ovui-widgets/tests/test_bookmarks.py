...
``name → url`` mapping via :class:`ovui_widgets.common.settings.Settings`) and the
...
from ovui_data_adapters.services.content.navigation import (
    BookmarksManager as ServiceBookmarksManager,
    SETTINGS_KEY as SERVICE_SETTINGS_KEY,
)
...
from ovui_widgets.content.bookmarks import (
    SETTINGS_KEY,
    BookmarksManager,
)
from ovui_widgets.content.widget.bookmark_button import BookmarkButton
...
class TestBookmarksManagerDefaults:
    def test_ovui_widgets_bookmarks_manager_is_compatibility_export(self):
        assert BookmarksManager is ServiceBookmarksManager
        assert SETTINGS_KEY == SERVICE_SETTINGS_KEY
    ...
    def test_service_bookmarks_manager_imports_without_ui_runtime(self):
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    "import sys; "
                    "from ovui_data_adapters.services.content.navigation "
                    "import BookmarksManager, SETTINGS_KEY; "
                    "from ovui_data_adapters.services.settings import Settings; "
                    "settings = Settings(); "
                    "manager = BookmarksManager(settings); "
                    "manager.add('Home', 'mock://Home'); "
                    "events = []; "
                    "sub = manager.subscribe_changed(lambda: events.append(manager.list())); "
                    "manager.add('Docs', 'mock://Docs'); "
                    "sub.cancel(); "
                    ...
                    "print(SETTINGS_KEY, manager.list(), events); "
        ...
        assert proc.returncode == 0, (
            "service BookmarksManager imported UI/runtime modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "ui.content.bookmarks" in proc.stdout
        assert "mock://Docs" in proc.stdout
