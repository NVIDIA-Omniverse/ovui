...
* :mod:`ovui_widgets.property.widget` — the new subpackage that re-exports the
  abstract :class:`PropertyWidget` base.
* :class:`ovui_widgets.property.widget.PropertyWidget` — the ABC cannot be
...
* :class:`ovui_widgets.property.window.PropertyWindow` — the renamed class is
  still a :class:`ovui_widgets.common.managed_window.ManagedWindow`, still
...
* Compat alias — ``from ovui_widgets.property import PropertyWidget`` still
  resolves to :class:`PropertyWindow` so
  :meth:`ovui_widgets.app.application.Application.create_windows` doesn't break.
* Widget iteration — registered widgets get :meth:`on_new_payload` and
  ...
  :class:`ovui_widgets.property.payload.PropertyPayload` built from the current
  selection.
"""
...
class TestPackageStructure:
    def test_widget_subpackage_importable(self):
        import ovui_widgets.property
        import ovui_widgets.property.widget  # noqa: F401

    def test_property_widget_abstract_module_importable(self):
        import ovui_widgets.property.widget.property_widget as mod
        assert mod.PropertyWidget is not None

    def test_widget_subpackage_reexports_base(self):
        from ovui_widgets.property.widget import PropertyWidget as Base
        from ovui_widgets.property.widget.property_widget import PropertyWidget as Direct
    ...
    def test_top_level_exports(self):
        import ovui_widgets.property
        assert hasattr(ovui_widgets.property, "PropertyPayload")
        assert hasattr(ovui_widgets.property, "PropertyWindow")
        assert hasattr(ovui_widgets.property, "PropertyWidget")


# ---------------------------------------------------------------------------
# Compat alias — ``from ovui_widgets.property import PropertyWidget`` → PropertyWindow
# ---------------------------------------------------------------------------
...
class TestCompatAlias:
    def test_top_level_propertywidget_is_propertywindow(self):
        ...
        :meth:`ovui_widgets.app.application.Application.create_windows` keeps
        working unchanged."""
        from ovui_widgets.property import PropertyWidget as Alias
        from ovui_widgets.property.window import PropertyWindow
