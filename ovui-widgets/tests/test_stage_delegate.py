...
class TestStageDelegateImport:
    ...
    def test_row_height_matches_design_token(self):
        ...
        import ovui_widgets.app.style.constants  # noqa: F401
    ...
    def test_prim_icon_size_is_smaller_than_dense_row(self):
        from ovui_widgets.stage.widget.stage_delegate import _ROW_HEIGHT, _TYPE_ICON_SIZE

        assert _TYPE_ICON_SIZE == 10
        assert _TYPE_ICON_SIZE < _ROW_HEIGHT

    def test_tree_chevron_size_matches_property_header(self):
        from ovui_widgets.property.group_widget import _CHEVRON_SIZE as property_size
        from ovui_widgets.stage.widget.stage_delegate import _CHEVRON_SIZE as tree_size

        assert tree_size == property_size == 12

    def test_default_prim_pill_label_is_reference_abbreviation(self):
        from ovui_widgets.stage.widget.stage_delegate import _DEFAULT_PILL_LABEL

        assert _DEFAULT_PILL_LABEL == "DEF"
        assert _DEFAULT_PILL_LABEL != "DEFAULT"

    def test_default_prim_pill_geometry_is_compact(self):
        from ovui_widgets.stage.widget.stage_delegate import (
    ...
    def test_exported_from_package(self):
        from ovui_widgets.stage import StageDelegate as SD
...
class TestSelectionAccentRendering:
    def _patch_row_ui(self, monkeypatch):
        from ovui_widgets.stage.widget import stage_delegate as delegate_mod
...
class TestBranchChevronRendering:
    def _patch_branch_ui(self, monkeypatch):
        ...
        class _FakeImage:
            ...
            def set_mouse_pressed_fn(self, callback):
                self.mouse_pressed_fn = callback
    ...
    def test_tree_branch_chevron_routes_left_press_to_explicit_toggle(
        self, monkeypatch
    ):
        _, images = self._patch_branch_ui(monkeypatch)
        model = HierarchyModel(MockStageAdapter())
        item = model.get_item_children(None)[0]
        toggles = []
        delegate = StageDelegate()
        delegate.set_expand_toggle_callback(
            lambda toggled_item, expanded: toggles.append(
                (toggled_item, expanded)
            )
        )

        delegate.build_branch(model, item, 0, 0, True)
        images[-1].mouse_pressed_fn(0, 0, 0, 0)

        assert toggles == [(item, False)]
