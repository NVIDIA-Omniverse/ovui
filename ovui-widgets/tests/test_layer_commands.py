...
from ovui_data_adapters.services.layers import commands as service_layer_commands
...
from ovui_widgets.layers import AbstractLayerCommand
import ovui_widgets.layers.commands as widgets_layer_commands
...
class TestImportSurface:
    def test_ovui_widgets_layer_commands_export_service_objects(self) -> None:
        for name in (
            ...
            assert (
                getattr(widgets_layer_commands, name)
                is getattr(service_layer_commands, name)
            )

    def test_services_layer_commands_import_without_ui_runtime_modules(self) -> None:
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                (
                    ...
                    "LAYERS_UNDO_SOURCE, SaveLayerCommand, SetEditTargetCommand; "
                    ...
                    "print(SaveLayerCommand.non_undoable, "
                    "SetEditTargetCommand.__name__, LAYERS_UNDO_SOURCE, forbidden); "
        ...
        assert proc.returncode == 0, (
            "layer command service imported UI/runtime modules:\n"
            f"--- stdout ---\n{proc.stdout}\n--- stderr ---\n{proc.stderr}"
        )
        assert "True SetEditTargetCommand ovui_widgets.layers:undo []" in proc.stdout
...
class TestBaseClassShape:
    ...
    def test_source_constants_namespaced(self) -> None:
        ...
        assert LAYERS_COMMAND_SOURCE == "ovui_widgets.layers:command"
        assert LAYERS_UNDO_SOURCE == "ovui_widgets.layers:undo"
