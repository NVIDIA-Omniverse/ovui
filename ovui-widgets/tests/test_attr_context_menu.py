...
* Right-click on an attribute row's outer ``ui.HStack`` routes through
  :func:`ovui_widgets.property.attribute_row._wire_row_context_menu` with
...
* ``can_paste`` / ``can_reset`` disable the menu items in the obvious
  dead-end cases (empty clipboard, type mismatch, locked, adapter does
  not advertise ``clear_values`` support, unauthored attr).

The tests exercise the pure helpers against a
:class:`~ovui_widgets.common.testing.mock_property.MockPropertyAdapter` (no USD,
...
class _UnimplementedClearAdapter(MockPropertyAdapter):
    ...
    def get_capabilities(self) -> PropertyCapabilities:
        return _property_capabilities(True)
...
class TestResetValue:
    ...
    def test_reset_on_abc_default_raises(self) -> None:
        """The ABC default capability is unsupported, so reset is disabled."""
...
class TestCanReset:
    ...
    def test_unsupported_capability_disables_reset(self) -> None:
        """Same class and clear implementation; only capability differs."""
        metas = {"a": _meta("a")}
        ...
        assert can_reset(supported, "a") is True
        assert can_reset(unsupported, "a") is False
...
class TestRowContextMenuWiring:
    ...
    def test_right_click_fires_context_menu_callback(self, monkeypatch) -> None:
        ...
        from ovui_widgets.property.attribute_row import _wire_row_context_menu
        from ovui_widgets.property.parts import attr_context_menu as ctx_mod
