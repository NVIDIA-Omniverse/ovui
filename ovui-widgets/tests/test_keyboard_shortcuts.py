...
class TestSaveStage:
    ...
    def test_no_stage_shows_error(self):
        app = _make_app()
        app._layer_adapter = None
        with patch("ovui_widgets.common.error_reporter.ErrorReporter") as Reporter:
            app.save_stage()
...
class TestFilterOwnedDeleteKeys:
    """A focused, editable Stage filter owns Backspace/Delete.

    Reproduced bug: with a Content Browser item selected and the Stage Browser
    filter focused at ``Cube``, one Backspace edited the filter to ``Cub`` and
    also opened Content Browser's Confirm Delete for the selected item. While
    the filter owns keyboard input, Backspace/Delete must not fire the
    destructive selection-delete fan-out (Layers prim-spec / Stage prim /
    Content Browser). When the filter is not editing, the intentional delete
    shortcut must still run — including the Layers-priority consume path.
    """

    @staticmethod
    def _app_with_targets(filter_editing):
        app = _make_app()
        stage_window = MagicMock()
        stage_window.is_filter_editing.return_value = filter_editing
        app._stage_window = stage_window
        app._content_window = MagicMock()
        app._delete_selected = MagicMock()
        app._delete_selected_prim_specs_in_layers = MagicMock(return_value=False)
        return app

    @pytest.mark.parametrize("key", [_KEY_BACKSPACE, _KEY_DELETE])
    def test_filter_editing_suppresses_all_destructive_targets(self, key):
        app = self._app_with_targets(filter_editing=True)
        app._on_key_pressed(key, 0, True)
        # No destructive target may fire while the filter owns the key —
        # not even the Layers consume path (guards against a helper that
        # merely swallows the key and hides an incomplete guard).
        app._delete_selected_prim_specs_in_layers.assert_not_called()
        app._delete_selected.assert_not_called()
        app._content_window.delete_selected.assert_not_called()

    @pytest.mark.parametrize("key", [_KEY_BACKSPACE, _KEY_DELETE])
    def test_filter_not_editing_preserves_stage_and_content_delete(self, key):
        app = self._app_with_targets(filter_editing=False)
        app._on_key_pressed(key, 0, True)
        app._delete_selected_prim_specs_in_layers.assert_called_once()
        app._delete_selected.assert_called_once()
        app._content_window.delete_selected.assert_called_once()

    @pytest.mark.parametrize("key", [_KEY_BACKSPACE, _KEY_DELETE])
    def test_filter_not_editing_preserves_layers_priority(self, key):
        app = self._app_with_targets(filter_editing=False)
        app._delete_selected_prim_specs_in_layers.return_value = True
        app._on_key_pressed(key, 0, True)
        # Layers consumed the key: stage/content delete must not run,
    ...
    @pytest.mark.parametrize("key", [_KEY_BACKSPACE, _KEY_DELETE])
    def test_no_stage_window_falls_back_to_delete(self, key):
        # Absent stage window (test/headless harness) must not suppress the
        # intentional delete shortcut.
        app = self._app_with_targets(filter_editing=False)
        app._stage_window = None
        app._on_key_pressed(key, 0, True)
        app._delete_selected.assert_called_once()
