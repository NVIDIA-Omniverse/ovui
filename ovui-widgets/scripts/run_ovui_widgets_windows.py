...
"""Windows launcher for ovui-widgets / OvGear.
...
shim does that bootstrap before handing off to ovui_widgets.app.__main__.

Usage (via the ovui-widgets-win.bat or ovgear-win.bat wrapper, or directly inside _venv312):
...
Env vars honored (all optional, sensible defaults):
    ...
    OVGEAR_ZERO_COPY   GPU viewport image upload toggle (default: "1";
                       set to "0" to force CPU readback)
"""
...
def _bootstrap() -> None:
    ...
    # Windows camera navigation misses the 60 Hz vsync budget when every
    # frame copies LdrColor through CPU. The ImageBridge probes support on
    # the first GPU frame and falls back automatically if the backend cannot
    # consume GPU pointers, so make the fast path the default while preserving
    # OVGEAR_ZERO_COPY=0 as an explicit opt-out.
    os.environ.setdefault("OVGEAR_ZERO_COPY", "1")
...
def main() -> None:
    _bootstrap()
    sys.argv[0] = "ovgear"
    runpy.run_module("ovui_widgets.app", run_name="__main__")
