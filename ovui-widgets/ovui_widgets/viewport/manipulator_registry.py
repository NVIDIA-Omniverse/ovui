...
:class:`~ovui_widgets.viewport.transform_manipulator.TransformManipulator` and cycles
...
* :class:`ManipulatorRegistry` — predates Phase C; routes selection changes
  ...
  :class:`~ovui_widgets.viewport.prim_transform_model.PrimTransformModel`.
...
# ``ovui_widgets.viewport/camera_flight_keyboard.py`` for the equivalent convention
# on W/A/S/D/Q/E).
_KEY_W = ord("W")
_KEY_E = ord("E")
_KEY_R = ord("R")
...
class ToolRegistry:
    ...
    * The attached :class:`~ovui_widgets.viewport.transform_manipulator.TransformManipulator`
