...
1. **Geometry** — :func:`build_rotate_gizmo` draws three ``sc.Arc`` rings
   ...
   :class:`~ovui_widgets.viewport.translate_gizmo.HighlightGesture` (hover).
2. **Drag math** — :class:`PrimRotateChangedGesture` reads the arc's signed
   ...
   :meth:`~ovui_widgets.viewport.prim_transform_model.PrimTransformModel.on_drag_rotated`.
...
3. **Highlight** — the same
   :class:`~ovui_widgets.viewport.translate_gizmo.HighlightGesture` swaps the ring's
...
:class:`~ovui_widgets.viewport.transform_manipulator.TransformManipulator` wraps
...
class PrimRotateChangedGesture(TransformGestureBase):
    ...
    * ``on_ended``: call :meth:`PrimTransformModel.on_drag_ended` which
    ...
    def __init__(
        ...
        # :class:`ovui_widgets.viewport.translate_gizmo.PrimTranslateChangedGesture`.
        ...
        # ``ovui_widgets.viewport.translate_gizmo.PrimTranslateChangedGesture`` for
        # the full rationale.
...
def build_rotate_gizmo(
    ...
    :func:`~ovui_widgets.viewport.translate_gizmo.build_translate_gizmo`.
