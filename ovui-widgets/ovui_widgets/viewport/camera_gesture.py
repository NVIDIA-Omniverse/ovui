...
from ovui_widgets.viewport.camera_inertia import AngularVelocityTracker, TumbleInertia
...
# Keep pan math inside the same practical distance envelope as
# CameraController. Tests can still directly assign invalid state; runtime
# gesture handling should ignore that instead of propagating NaN/inf targets.
_MAX_PAN_DISTANCE = 1.0e6
...
class _AngularDragGesture(sc.DragGesture):
    ...
    def is_active(self) -> bool:
        ...
        Exposed publicly so :class:`~ovui_widgets.viewport.camera_flight_keyboard.FlightModeKeyboard`
...
class TumbleGesture(_AngularDragGesture):
    ...
    :class:`~ovui_widgets.viewport.camera_manipulator.CameraManipulator` instantiates
    two configurations of this class: the default (``MOUSE_RIGHT,
    MOD_NONE``) and an Alt+LMB variant (``MOUSE_LEFT, MOD_ALT``). Both
    share the same :class:`~ovui_widgets.viewport.camera_inertia.TumbleInertia`
...
class PanGesture(sc.DragGesture):
    ...
    def __init__(
        ...
        self._prev_anchored = False
        self._active = False
        self._world_per_ndc_x = 0.0
        self._world_per_ndc_y = 0.0
        self._pan_scale_valid = False
        self.set_on_began_fn(self._on_began)
        self.set_on_changed_fn(self._on_changed)
        self.set_on_ended_fn(self._on_ended)
    ...
    def _on_changed(self, sender: Any = None) -> None:
        ...
        self._prev_x = m.x
        self._prev_y = m.y
        if not self._pan_scale_valid:
            self._cache_pan_scale()
            if not self._pan_scale_valid:
                return
        dx_world = -dx_ndc * self._world_per_ndc_x
        dy_world = -dy_ndc * self._world_per_ndc_y
        self._camera.pan(dx_world, dy_world)

    def _cache_pan_scale(self) -> None:
        self._pan_scale_valid = False
        ...
        try:
            ...
            width_f = 0.0
            height_f = 0.0
        aspect = (width_f / height_f) if (width_f > 0 and height_f > 0) else 1.0
        if not math.isfinite(aspect) or aspect <= 0.0:
            aspect = 1.0
        try:
            distance = float(self._camera.state.distance)
        except (TypeError, ValueError):
            return
        if not math.isfinite(distance):
            return
        distance = max(min(distance, _MAX_PAN_DISTANCE), 0.0)
        world_per_ndc_y = distance * math.tan(self._fov_y * 0.5)
        world_per_ndc_x = world_per_ndc_y * aspect
        if not math.isfinite(world_per_ndc_x) or not math.isfinite(world_per_ndc_y):
            return
        self._world_per_ndc_x = world_per_ndc_x
        self._world_per_ndc_y = world_per_ndc_y
        self._pan_scale_valid = True
