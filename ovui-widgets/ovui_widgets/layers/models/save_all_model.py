...
- :meth:`get_value_as_bool` walks the adapter's
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.get_layer_stack_identifiers`
...
- :meth:`get_dirty_identifiers` returns the same filtered list
  ...
  pushes one :class:`~ovui_widgets.layers.commands.SaveLayerCommand` per
...
- :meth:`set_value` delegates to
  :meth:`~ovui_widgets.layers.layer_model.LayerModel._request_save_all`. The
  argument is ignored (same "do it" contract as
  :class:`~ovui_widgets.layers.models.save_model.SaveValueModel`). A no-op
...
by :meth:`~ovui_widgets.layers.layer_model.LayerModel._flush_events` whenever
