...
:func:`ovui_widgets.app.style.apply_global_styles` is re-called on theme change.
...
LAYERS_STYLES: dict = {
    ...
    # at ``ovui_widgets.stage/style.py:124``) so the two panels' selected rows
    ...
    # /  ``chevron_down.png`` from ``ovui_widgets.app/resources/icons/``) via
    ...
    # ``Stage.TreeChevron`` at ``ovui_widgets.stage/style.py:138-140`` one-for-one
    ...
    # override whenever the :class:`~ovui_widgets.layers.drop_visual_controller.
