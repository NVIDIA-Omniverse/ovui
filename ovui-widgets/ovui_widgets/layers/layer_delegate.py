...
:class:`~ovui_widgets.layers.models.layer_name_model.LayerNameValueModel` —
...
:class:`~ovui_widgets.layers.models.save_model.SaveValueModel`: a small dot
...
:meth:`~ovui_widgets.common.adapters.LayerStackAdapter.save_layer`. Phase F
wraps the adapter call in a ``SaveLayerCommand`` for undo support.

Step 20 paints column 3 through
:class:`~ovui_widgets.layers.models.mute_model.LocalMuteValueModel`: a filled
...
:class:`~ovui_widgets.layers.models.lock_model.LockValueModel`: a small
...
from ovui_widgets.layers.drop_visual_controller import (
    INDICATOR_DROP_ABOVE,
    INDICATOR_DROP_REJECTED,
    INDICATOR_DROP_TARGET,
    INDICATOR_NONE,
)
from ovui_widgets.layers.layer_icons import (
    composition_badge,
    instance_badge,
    provider,
    specifier_icon,
...
# Branch chevron PNGs. Mirror ``ovui_widgets.stage/widget/stage_delegate.py``
# one-for-one — same files, same on-disk path — so the Stage and Layers
# tree branch arrows render identically. ``ovui_widgets/common/icons/`` ships
...
# ``ovui_widgets.stage/widget/stage_icons.py::provider``.
...
# :class:`ovui_widgets.layers.context_menu.MenuContext` and shows the menu via
...
RowLeftClickCallback = Callable[[Any, int], None]
ExpandToggleCallback = Callable[[Any, bool], None]
...
class LayerDelegate(ui.AbstractItemDelegate):
    ...
    # The row-level right-click surface consumes mouse presses before
    # ``ui.TreeView`` sees them. Route left presses and branch toggles back
    # through the owning window so selection, keyboard focus, and expansion
    # remain real user interactions instead of being shadowed by the menu hit
    # target.
    on_left_click: Optional[RowLeftClickCallback] = None
    on_toggle_expand: Optional[ExpandToggleCallback] = None
    ...
    def _build_focus_ring(self, item: Any) -> None:
        ...
        # The ring is an observation-only overlay. Without an explicit
        # pointer-transparent flag it sits above the save/mute/lock controls
        # in the cell ZStack and can consume the next click on a selected row.
        ring.opaque_for_mouse_events = False
    ...
    def _build_drop_indicator(self, indicator: str) -> None:
        ...
        :mod:`ovui_widgets.layers.drop_visual_controller`. Rendering strategy:
        ...
        with ui.ZStack():
            ...
            with ui.HStack():
                ...
                if has_children:
                    ...
                    # ``ovui_widgets.stage/widget/stage_delegate.py:88-99``).
                    ...
                    with ui.VStack(width=ui.Pixel(self._CHEVRON_SIZE)):
                        ...
                        if self.on_toggle_expand is not None:
                            toggle = self.on_toggle_expand
    ...
    def build_widget(
        ...
        if isinstance(item, PrimSpecItem):
            ...
            if column_id == self.COL_NAME and self.on_left_click is not None:
                left_cb = self.on_left_click
                cell_stack.set_mouse_pressed_fn(
                    lambda x, y, btn, mod, _item=item, _cb=left_cb: (
                        _cb(_item, int(mod)) if btn == 0 else None
                    )
                )
            return
        if not isinstance(item, LayerItem):
        ...
        row_hit_columns = {
            self.COL_NAME,
            self.COL_LIVE,
            self.COL_GLOBAL_MUTE,
            self.COL_LATEST,
        }
        if column_id in row_hit_columns and (
            self.on_right_click is not None or self.on_left_click is not None
        ):
            right_cb = self.on_right_click
            left_cb = self.on_left_click
            ...
            def _on_cell_pressed(
                ...
                mod: int,
                *,
                _item: LayerItem = item,
                _right: Optional[RightClickCallback] = right_cb,
                _left: Optional[RowLeftClickCallback] = left_cb,
            ) -> None:
                if btn == 0 and _left is not None:
                    _left(_item, int(mod))
                elif btn == 1 and _right is not None:
                    _right(_item, x, y)
    ...
    def _build_prim_spec_name_widget(self, item: PrimSpecItem) -> None:
        ...
        PNG glyph chosen by :func:`ovui_widgets.layers.layer_icons.specifier_icon`:
        ...
        that bypasses :func:`ovui_widgets.common.style.urls.register_urls` or a
    ...
    def _build_prim_spec_icon(self, item: PrimSpecItem) -> None:
        ...
        :func:`ovui_widgets.common.style.urls.register_urls`) the method falls back
    ...
    def _build_leading_icon(self, state: str) -> None:
        ...
        overrides declared in :mod:`ovui_widgets.layers.style`.
        """
        with ui.VStack(
    ...
    def _build_local_mute_widget(self, model: Any, item: LayerItem) -> None:
        ...
        def _on_mute_pressed(
            x: float,
            y: float,
            btn: int,
            mod: int,
            vm: LocalMuteValueModel = value_model,
        ) -> None:
            if btn != 0:
                return
            current = vm.get_value_as_bool()
            vm.set_value(not current)

        stack.set_mouse_pressed_fn(_on_mute_pressed)
