...
Mirrors :class:`ovui_widgets.stage.window.StageWindow` in structure: the window
...
:class:`ovui_widgets.common.adapters.LayerStackAdapter` — :meth:`set_adapter`
...
from ovui_widgets.layers.context_menu import ContextMenuBuilder, MenuContext
...
from ovui_widgets.layers.selection_watch import LayerSelectionWatch
...
# (no SVG support), so we reuse the raster PNGs ``ovui_widgets.common.style.urls``
...
def _layers_filter_icon_provider(name: str) -> Any:
    ...
    Same pattern as :func:`ovui_widgets.property.window._filter_icon_provider` —
...
class LayerWindow(ManagedWindow):
    ...
    def __init__(
        ...
        self._selection_anchor: Any = None
    ...
    def _build_ui(self) -> None:
        ...
        if self._save_all_sub is not None:
            ...
            # ``cancel`` used by ovui_widgets.common.settings.Subscription). Call
        ...
        if self._model is None:
            self._model = LayerModel(
                ...
                before_save_all_fn=self._before_save_all_fn,
            )
        ...
        self._delegate.on_left_click = self._on_row_left_click
        self._delegate.on_toggle_expand = self._set_row_expanded
        with ui.VStack(spacing=0):
            ...
            self._tree_scrolling_frame.raster_policy = ui.RasterPolicy.AUTO
    ...
    def _build_filter_bar(self) -> None:
        ...
        ``StageWidget.build`` (``ovui_widgets.stage/widget/stage_widget.py:107-204``)
    ...
    def _on_filter_changed(self, model: Any) -> None:
        ...
        pattern ``ovui_widgets.property.PropertyWindow`` uses.
    ...
    def _on_filter_begin_edit(self, model: Any) -> None:
        ...
        (``ovui_widgets.stage/widget/stage_widget.py:322-326``). omni.ui's
    ...
    def set_adapter(self, adapter: Optional[LayerStackAdapter]) -> None:
        ...
        Called by :class:`ovui_widgets.app.application.Application` after a stage
    ...
    def refresh_layer_contents(self) -> None:
        ...
        # The model emits both value changes and a root ``item_changed``
        # notification. Let the existing TreeView rebuild affected rows in
        # place: replacing the whole frame here loses native row identity and
        # briefly leaves selected action controls unpainted/unreachable.
    ...
    def _visible_tree_items(self) -> List[Any]:
        """Return rows in the same visible order as the live TreeView."""
        ...
        def walk(parent: Any) -> None:
            for child in list(self._model.get_item_children(parent) or []):
                visible.append(child)
                if self._tree_view.is_expanded(child):
                    walk(child)

        walk(None)
        return visible

    def _on_row_left_click(self, item: Any, modifiers: int) -> None:
        """Restore row selection consumed by the context-menu hit surface.

        The delegate owns a per-cell mouse callback so it can open a menu on
        right-click. In ovui that callback also consumes left presses before
        ``ui.TreeView`` updates its selection. Reapply the normal single,
        Ctrl-toggle, and Shift-range semantics through the public TreeView
        selection property.
        """
        ...
        if tree is None:
            return
        current = list(getattr(tree, "selection", ()) or ())
        if modifiers & _MOD_SHIFT and self._selection_anchor is not None:
            visible = self._visible_tree_items()
            try:
                start = visible.index(self._selection_anchor)
                ...
                selected = visible[lower : upper + 1]
        elif modifiers & _MOD_CTRL:
            selected = [candidate for candidate in current if candidate is not item]
            if item not in current:
                selected.append(item)
                self._selection_anchor = item
            elif self._selection_anchor is item:
                self._selection_anchor = selected[-1] if selected else None
        else:
            selected = [item]
            self._selection_anchor = item
        tree.selection = selected

    def _set_row_expanded(self, item: Any, expanded: bool) -> None:
        """Apply an explicit branch-chevron gesture to the live TreeView."""

        tree = self._tree_view
        if tree is not None:
            tree.set_expanded(item, bool(expanded), False)
    ...
    def get_selected_items(self) -> List[Any]:
        ...
        into the model from :class:`~ovui_widgets.app.application.Application`.
