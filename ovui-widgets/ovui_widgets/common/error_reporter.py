...
class ErrorReporter:
    ...
    def show_status(cls, message: str, duration_ms: int = 3000, level: str = "") -> None:
        ...
        When initialized with a StatusBar, route through that widget so it
        controls status-frame visibility and auto-dismiss. When only a label
        is initialized, set its text/style directly and schedule auto-dismiss
        via app.call_later(). duration_ms=0 means no auto-dismiss. Message
        text is flattened to a single line and truncated via
        :meth:`_sanitize_status_text` before display, so multi-line error
        strings cannot wrap into adjacent panels.
        """
        display = cls._sanitize_status_text(message)
        if cls._status_bar is not None:
            cls._status_bar.show_message(display, duration_ms, level)
        elif cls._status_label is not None:
