...
_LOG = logging.getLogger("ovui_widgets.app.input_bridge")
...
class RemoteInputBridge:
    ...
    def __init__(self, width: int, height: int) -> None:
        ...
        self._mouse_modifier_mask: int = 0
        self._mouse_modifier_dirty: bool = False
    ...
    def on_mouse_event(self, event: MouseEvent) -> None:
        mouse_modifier_mask = int(event.modifiers)
        if event.type == MouseEventType.BUTTON and event.button_state == KeyState.UP:
            mouse_modifier_mask = 0
        if event.type == MouseEventType.MOVE:
            x, y = self._clamp(event.x, event.y)
            with self._lock:
                self._mouse_xy = (x, y)
                self._set_mouse_modifier_mask_unlocked(mouse_modifier_mask)
            return
        ...
        if event.type == MouseEventType.WHEEL:
            with self._lock:
                ...
                if (
                    ...
                    tail.scroll_x += event.scroll_x
                    tail.scroll_y += event.scroll_y
                    return
    ...
    def mouse_modifier_mask(self) -> int:
        """Return the latest mouse-event modifier mask."""
        with self._lock:
            return self._mouse_modifier_mask

    def consume_mouse_modifier_state(self) -> Tuple[int, bool]:
        """Return ``(mask, changed)`` and clear the changed flag."""
        with self._lock:
            mask = self._mouse_modifier_mask
            changed = self._mouse_modifier_dirty
            self._mouse_modifier_dirty = False
        return mask, changed
    ...
    def _set_mouse_modifier_mask_unlocked(self, mask: int) -> None:
        mask = int(mask)
        if mask != self._mouse_modifier_mask:
            self._mouse_modifier_mask = mask
            self._mouse_modifier_dirty = True
