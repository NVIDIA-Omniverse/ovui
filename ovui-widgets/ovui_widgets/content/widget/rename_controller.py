...
See the content browser behavior (rename surface) and the content browser implementation step 33. Mirrors the ovui_widgets.stage :class:`RenameController` pattern
(``ovui_widgets.stage/widget/rename_controller.py``): a single object owns the
...
:class:`ovui_widgets.common.error_reporter.ErrorReporter` using the same warning /
...
class RenameController:
    ...
    * **Active** — ``_active_item is <FileItem>``. The name column for
    ...
    def __init__(self, widget: "FileBrowserWidget") -> None:
        ...
        # matching the ovui_widgets.stage surface.
