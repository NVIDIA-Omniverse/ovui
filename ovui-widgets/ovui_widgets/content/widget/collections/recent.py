...
:class:`ovui_widgets.common.recent_files.RecentFileList`; the owning
:class:`ovui_widgets.app.application.Application` writes to it on every
:meth:`~ovui_widgets.app.application.Application.open_file` call and persists
...
# :class:`ovui_widgets.app.application.Application` writes to this key on every
# ``open_file`` call and the key is where persistence round-trips
# through :class:`ovui_widgets.common.settings.Settings._load_from_disk`. Match
...
class RecentFilesCollection(CollectionItem):
    ...
    :meth:`ovui_widgets.common.recent_files.RecentFileList.get_ordered` — most
    ...
    :class:`ovui_widgets.content.widget.collections.my_computer.MyComputerCollection`
    docstring for the pointer-identity constraint).

    Change propagation mirrors
    :class:`ovui_widgets.content.widget.collections.bookmarks.BookmarksCollection`:
