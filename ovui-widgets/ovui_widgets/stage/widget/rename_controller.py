...
class RenameController:
    ...
    def __init__(
        ...
        # Step 5). Defaults to :func:`ovui_widgets.common.scheduler.call_later`
        ...
        if name and name != current:
            try:
                ...
                ErrorReporter.show_error(
                    f"Cannot rename prim: {type(exc).__name__}: {exc}"
                )
        self._end_rename()
...
def _default_call_later(delay_secs: float, callback: Callable) -> Any:
    ...
    Lazy-imports :mod:`ovui_widgets.common.scheduler` so the
