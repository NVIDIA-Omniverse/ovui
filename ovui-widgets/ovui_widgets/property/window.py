...
:class:`ovui_widgets.property.widget.PropertyWidget` sections. The window owns the
filter bar and the scrollable container; each widget returned by
:class:`~ovui_widgets.property.widget.PropertySchemeRegistry` for the current
...
:mod:`ovui_widgets.property` for one release cycle so existing callers (notably
:class:`ovui_widgets.app.application.Application`) keep working. The new abstract
:class:`ovui_widgets.property.widget.PropertyWidget` base is the class Step 6.2 +
onwards subclasses.

Step 6.2 of the property inspector implementation moved the group-tree build + row dispatch
into :class:`~ovui_widgets.property.widget.AttributesWidget`. Step 6.5 hoisted
...
# raster ``.png`` variants that ``ovui_widgets.stage`` already ships are re-used
# through a tiny module-scoped cache. See ``ovui_widgets.stage.widget.stage_icons``
...
class PropertyWindow(ManagedWindow):
    ...
    def __init__(self) -> None:
        ...
        # Read-only row registry used by screenshot-first Inspector QA to
        # locate the real value fields it must click. Rows are rebuilt with
        # the Property panel and never provide a mutation shortcut; tests
        # still use ordinary mouse/keyboard input.
        self._inspector_attribute_rows: Dict[str, Any] = {}
        self._filter_text = ""
    ...
    def _on_stage_changed(self, event: Any) -> None:
        ...
        def affects(changed: str, sel: str) -> bool:
            if changed.startswith(sel):
                return True
            # A bare prim entry is a coarse root (e.g. a Mode B replay
            # resync): the selection's own locally-authored rows may have
            # changed anywhere under it, so an ancestor-or-self root
            # rebuilds the panel. Property entries name exactly one prim
            # and never widen to descendants.
            if "." in changed:
                return False
            return sel == changed or sel.startswith(changed.rstrip("/") + "/")

        affected = any(
            affects(str(changed), sel)
    ...
    def _schedule_deferred_rebuild(self) -> None:
        ...
        :class:`~ovui_widgets.app.application.Application` singleton is live so
    ...
    def destroy(self) -> None:
        ...
        self._release_attribute_rows()
        self._inspector_attribute_rows = {}
        super().destroy()

    def _release_attribute_rows(self) -> None:
        """Revoke every obsolete row subscription (rebuild/teardown).

        Each Property Inspector row owns one adapter subscription; a
        rebuild, filter/selection refresh, or window destruction must
        explicitly release it — dropping the row dictionary alone would
        leave the stage adapter holding a live callback per row.
        """
        rows = getattr(self, "_inspector_attribute_rows", None) or {}
        for row in list(rows.values()):
            sub = getattr(row, "_adapter_sub", None)
            if sub is not None:
                try:
                    sub.cancel()
                except Exception:
                    pass  # owner-side retention keeps it revocable
                try:
                    row._adapter_sub = None
                except Exception:
                    pass
            try:
                row._value_sub = None
    ...
    def _build_registered_widgets(self) -> None:
        ...
        Step 6.5: asks
        :class:`~ovui_widgets.property.widget.PropertySchemeRegistry` for the
    ...
    # :mod:`ovui_widgets.property.widget.attributes_widget`; these one-liners are
