...
:class:`~ovui_widgets.property.widget.PropertyWidget` subclass that renders the
group tree + attribute rows :class:`~ovui_widgets.property.window.PropertyWindow`
...
:class:`~ovui_widgets.property.widget.SimplePropertyWidget`. The catch-all
...
(:func:`ovui_widgets.property.widget.scheme_registry._register_defaults`) so
...
class AttributesWidget(SimplePropertyWidget):
    ...
    schema and accepts every :class:`~ovui_widgets.property.payload.PropertyPayload`.
    ...
    def build_items_content(self) -> None:
        ...
        :class:`~ovui_widgets.property.group_widget.AttributeGroupWidget`.
        """
        self._build_groups()
    ...
    def _show_group_context_menu(
        ...
        :func:`ovui_widgets.property.parts.group_context_menu.show_group_context_menu`
        ...
        if adapter is None:
            return
        from ovui_widgets.property.parts.group_context_menu import show_group_context_menu
    ...
    def _build_attribute_row(self, prop: AttributeMetadata) -> None:
        ...
        row = WidgetBuilderTable.build(prop.name, prop, adapter, match=match)
        # Keep a bounded, selection-lifetime handle to the concrete row so
        # the Inspector state oracle can report its on-screen geometry. This
        # does not expose an action API: the QA harness still clicks/types
        # through ovui's normal input path.
        rows = getattr(self._window, "_inspector_attribute_rows", None)
        if rows is None:
            rows = {}
            self._window._inspector_attribute_rows = rows
        rows[prop.name] = row
