...
forwarder to :class:`ovui_widgets.property.builders.WidgetBuilderTable`. Step 1.3
...
from ovui_widgets.property.group_widget import _CHEVRON_DOWN
from ovui_widgets.property.models import AttributeModelBase
from ovui_widgets.property.parts import ControlStateIndicator, HighlightLabel
...
# in ``ovui_widgets/app/style/constants.py`` mirrors this value for any
...
def _set_value_field_focused(widget: Any, focused: bool) -> None:
    ...
    ``::focused`` in ``ovui_widgets/property/style.py`` — both must paint the
...
def _label_kwargs_from_metadata(prop: AttributeMetadata) -> Dict[str, Any]:
    ...
    ``::not_authored`` state selector (see :mod:`ovui_widgets.property.style`) can
...
def _wire_row_context_menu(hstack: Any, owner: Any) -> None:
    ...
    :func:`ovui_widgets.property.parts.attr_context_menu.show_attr_context_menu`.
    ...
    :mod:`ovui_widgets.property.parts` are only touched when a user actually
    right-clicks a row, not at module import time.
    """
    # Keep read-only geometry available to the Inspector. This does not expose
    # a mutation seam: the actual context-menu invocation still travels
    # through the row's real right-click handler below.
    owner._row_hstack = hstack
...
def _build_component_separator() -> ui.Rectangle:
    ...
    margin) lives on :data:`ovui_widgets.property.style.PROPERTY_STYLES` —
...
class TokenAttributeRow:
    ...
    The builder in ``ovui_widgets.property/builders/token.py`` dispatches here when
    ...
    def _on_item_changed(self, item_model: Any, item: Any) -> None:
        ...
        except BaseException as primary:  # noqa: BLE001 — structured cancel
            # The commit failed mid-transaction: cancel so the adapter
            # restores its exact baseline; the primary throwable stays
            # primary (a cancel failure is attached as a note).
            try:
...
# eagerly, which the ovui_widgets.property layer must not transitively pull in.
...
def _normalize_array_element(x: Any) -> Any:
    ...
    to import ``pxr.Gf`` (and the ovui_widgets.property module stays USD-free).
    """
...
def build_attribute_row(
    ...
    :class:`ovui_widgets.property.builders.WidgetBuilderTable`. The forwarder
