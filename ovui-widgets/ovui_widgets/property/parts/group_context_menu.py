...
* **Reset All** — calls ``adapter.clear_value(attr_name)`` on every
  attr in the group. Adapters that do not explicitly advertise the
  ``clear_values`` property capability are filtered out by
...
def reset_group(
    ...
    Adapters that do not advertise ``clear_values`` support are detected
    up-front via :func:`can_reset` so the menu item disables. The driver
    still catches ``NotImplementedError`` inline in case a third-party
    adapter's ``clear_value`` silently raises on one specific attr (e.g.
    relationship types in the USD adapter, which the property inspector
    behavior notes are skipped today).

    Returns the count of attrs successfully reset.
    """
    if not can_reset(adapter, group):
        return 0
    reset = 0
    for prop in iter_group_props(group):
        if prop.is_locked:
...
def can_reset(
    ...
    or when every attr in the group is locked or unauthored. Mirrors the
    NotDefault predicate in
    :mod:`ovui_widgets.property.parts.control_state`: "authored + adapter can
