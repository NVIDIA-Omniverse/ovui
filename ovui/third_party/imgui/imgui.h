...
#ifdef IMGUI_NVIDIA
    ImGuiCol_CustomChar,            // Color to render custom char
#endif
    ImGuiCol_COUNT,
...
struct ImGuiStyle
{
    ...
    bool        DockingTabBarUseTabColors;  // Use tab colors for dock-node tab bar backgrounds instead of title-bar colors. Disabled preserves default ImGui behavior.
    bool        DockingTabBarUseSelectedColorForSingleTab; // When DockingTabBarUseTabColors is enabled, use selected tab color for a single-tab dock-node background.
    bool        DockingTabBarShowInactiveTabSeparators; // Draw a separator between adjacent inactive dock-node tabs. Disabled preserves default ImGui behavior.
    float       DockingTabInactiveSeparatorInset; // Vertical inset for inactive dock-node tab separators. Only used when DockingTabBarShowInactiveTabSeparators is enabled.
    float       DockingTabBarHeight;        // Fixed height for dock-node document tab bars. 0.0f uses standard FramePadding-derived height.
    bool        TabUseRectangularShape;     // Draw zero-rounded tabs as simple rectangles. Disabled preserves default ImGui tab path drawing.
    ...
    ImU16       CustomCharBegin;            // Code of first custom char. Custom char will be rendered with ImGuiCol_CustomChar. 0xFFFF means no custom char.
#endif
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddShadowRect(const ImVec2& obj_min, const ImVec2& obj_max, ImU32 shadow_col, float shadow_thickness, const ImVec2& shadow_offset, ImDrawShadowFlags shadow_flags = 0, float obj_rounding = 0.0f, ImDrawFlags obj_rounding_corners = ImDrawFlags_RoundCornersAll);
...
struct ImFontAtlasShadowTexConfig
{
    ...
    float   TexDistanceFieldOffset; // How much to offset the distance field by.
