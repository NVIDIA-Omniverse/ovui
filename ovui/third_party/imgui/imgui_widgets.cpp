...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    if ((flags & ImGuiTabBarFlags_DockNode) && g.Style.DockingTabBarHeight > 0.0f)
        tab_bar->FramePadding.y = ImMax(0.0f, (tab_bar->BarRect.GetHeight() - g.FontSize) * 0.5f);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if ((tab_bar->Flags & ImGuiTabBarFlags_DockNode) && g.Style.DockingTabBarHeight > 0.0f)
    ...
    if (is_visible)
    {
        ...
        if ((tab_bar->Flags & ImGuiTabBarFlags_DockNode) && style.DockingTabBarShowInactiveTabSeparators && !tab_contents_visible)
        {
            const int tab_n = tab_bar->Tabs.index_from_ptr(tab);
            const bool has_next_tab = tab_n + 1 < tab_bar->Tabs.Size;
            const bool next_tab_inactive = has_next_tab && tab_bar->Tabs[tab_n + 1].ID != tab_bar->SelectedTabId;
            if (next_tab_inactive)
            {
                const float x = bb.Max.x - 0.5f;
                const float inset = ImMax(0.0f, style.DockingTabInactiveSeparatorInset);
                display_draw_list->AddLine(ImVec2(x, bb.Min.y + inset), ImVec2(x, bb.Max.y - inset), GetColorU32(ImGuiCol_Separator));
            }
        }
        RenderNavCursor(bb, id);
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    const float y1 = (g.Style.TabUseRectangularShape && rounding <= 0.0f) ? bb.Min.y : bb.Min.y + 1.0f;
    const float y2 = bb.Max.y - g.Style.TabBarBorderSize;
    if (g.Style.TabUseRectangularShape && rounding <= 0.0f)
    {
        draw_list->AddRectFilled(ImVec2(bb.Min.x, y1), ImVec2(bb.Max.x, y2), col);
        if (g.Style.TabBorderSize > 0.0f)
            draw_list->AddRect(ImVec2(bb.Min.x + 0.5f, y1 + 0.5f), ImVec2(bb.Max.x - 0.5f, y2), GetColorU32(ImGuiCol_Border), 0.0f, 0, g.Style.TabBorderSize);
        return;
    }
