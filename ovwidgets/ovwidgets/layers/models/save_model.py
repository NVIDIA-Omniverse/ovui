...
- :meth:`set_value` delegates to
  :meth:`~ovwidgets.layers.layer_model.LayerModel._request_save`, which
  Step 34 uses to push a :class:`SaveLayerCommand` through the owning
  :class:`~ovwidgets.app.application.Application`'s
  :class:`~ovwidgets.common.undo.UndoManager`. Step 36 extends the anonymous
...
:meth:`~ovwidgets.layers.layer_item.LayerItem.refresh_flags`). When the
