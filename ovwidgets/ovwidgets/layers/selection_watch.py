...
clicks to :class:`~ovwidgets.common.selection.SelectionBus` (Step 56), and
...
selection that contains a :class:`~ovwidgets.layers.prim_spec_item.PrimSpecItem`
...
from ovwidgets.common.selection import SelectionBusError
from ovwidgets.common.settings import Subscription
from ovwidgets.layers.commands.base import LAYERS_COMMAND_SOURCE, LAYERS_UNDO_SOURCE
...
if TYPE_CHECKING:  # pragma: no cover — type-hint guard
    from ovwidgets.common.selection import SelectionBus, SelectionChangedEvent
    from ovwidgets.layers.layer_model import LayerModel


#: Namespaced source string published by :meth:`_forward_prim_specs`.
#:
#: LAYERS-PLAN Step 56 pins this to ``"ovwidgets.layers:select"`` so Stage /
...
#: feedback loop.
LAYERS_SELECT_SOURCE = "ovwidgets.layers:select"
...
#: :class:`~ovwidgets.layers.commands.base.AbstractLayerCommand`) loop the
...
#: :meth:`ovwidgets.app.application.Application.call_later` so production code
