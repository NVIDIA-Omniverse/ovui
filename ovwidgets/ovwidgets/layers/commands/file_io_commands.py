...
command pipeline so they inherit uniform error reporting,
selection-snapshot capture, and dialog guards.

- :class:`SaveLayerCommand` — non-undoable. Persist a layer to disk
  via :meth:`~ovwidgets.common.adapters.LayerStackAdapter.save_layer`. On
  :class:`IOError` / :class:`PermissionError` the exception is
  surfaced to the user through :class:`~ovwidgets.common.error_reporter.ErrorReporter`
  and swallowed — the adapter keeps the layer's dirty bit set so
  the row still offers a retry affordance.
...
- :class:`SaveLayerAsCommand` (Step 36) — **partially** undoable. The
  ...
  :meth:`~ovwidgets.common.adapters.LayerStackAdapter.save_layer_as` would
  fail because the path already exists on disk.

:class:`SaveLayerCommand` and :class:`ReloadLayerCommand` set
:attr:`~ovwidgets.common.undo.Command.non_undoable` to ``True``.
:meth:`~ovwidgets.common.undo.UndoManager.push` honours this marker by
executing :meth:`~ovwidgets.common.undo.Command.do`, clearing the redo stack,
...
class SaveLayerCommand(AbstractLayerCommand):
    ...
    ``do_impl`` forwards to
    :meth:`~ovwidgets.common.adapters.LayerStackAdapter.save_layer`. A
    ...
    through :class:`~ovwidgets.common.undo.UndoManager`.
    """
    ...
    def _resolve_reporter(self) -> Any:
        ...
        # Fall back to the classmethod-based module-level reporter so
        # call sites that do not thread a reporter through still get
        # stderr / status-bar surfacing.
        try:
...
class ReloadLayerCommand(AbstractLayerCommand):
    ...
    ``do_impl`` forwards to
    :meth:`~ovwidgets.common.adapters.LayerStackAdapter.reload_layer`, which
    ...
    def __init__(
        ...
        # via :class:`~ovwidgets.common.undo.CommandCancelled`. A reload on a
...
class SaveLayerAsCommand(AbstractLayerCommand):
    ...
    1. **File write** via
       :meth:`~ovwidgets.common.adapters.LayerStackAdapter.save_layer_as`
    ...
    If :meth:`~ovwidgets.common.adapters.LayerStackAdapter.save_layer_as`
    ...
    :class:`~ovwidgets.common.error_reporter.ErrorReporter`. A subsequent
