...
"""Asset-type catalog — extension → (category, display name, icon URL key).

See the content browser implementation step 4 and the content browser behavior

This is the **single source of truth** for file-type recognition used by
every later content-browser component that needs to colour / icon /
filter / sort by file type. It is the ovgear equivalent of Kit's
``omni.kit.helper.file_utils/asset_types.py`` (architecture §32.1),
collapsed to a narrower category set appropriate for ovgear v1.
...
- Adds an :attr:`AssetCategory.ARCHIVE` category (Kit treats archives
  as Unknown — see §32.1 "Notable omissions").
- Adds an :attr:`AssetCategory.TEXT` category (Kit treats ``.txt`` /
  ``.md`` as Unknown).

The module is stdlib-only and has zero forward dependencies: no
``omni.ui``, no ``ovwidgets.content.backends``, no filesystem access. The
:attr:`AssetTypeDef.icon_url_key` strings line up with the URL keys
registered by ``ovwidgets.common.style.urls`` in Step 5.
"""
...
class AssetCategory(Enum):
    ...
    FOLDER = "folder"
    USD = "usd"          # .usd/.usda/.usdc/.usdz/.live/.omni and any dotted variant
    ...
    ARCHIVE = "archive"  # .zip/.tar/.gz/… (ovgear-specific; Kit has no archive type)
...
class AssetTypeDef:
    ...
    - ``icon_url_key``: attribute name on ``omni.ui.url`` that resolves
      to the icon for this category (e.g. ``"asset_usd"``). Registered
      by ``ovwidgets.common.style.urls`` in Step 5.
...
def get_icon_url_key(filename_or_url: str) -> str:
    """Return the ``omni.ui.url`` attribute name for ``filename_or_url``'s icon.

    The returned string is a key like ``"asset_usd"`` or
    ``"asset_unknown"``; the URL itself is registered by
    ``ovwidgets.common.style.urls`` (Step 5). This function never returns an
    empty string — :attr:`AssetCategory.UNKNOWN` always resolves to
    ``"asset_unknown"``.
    """
...
def register_extensions(category: AssetCategory, extensions: Iterable[str]) -> None:
    ...
    V1 ovgear has no plug-in system, so this is purely forward-looking:
    a future plug-in that adds a new image format can register it here
    without editing the built-in catalog. :attr:`AssetCategory.FOLDER`
    and :attr:`AssetCategory.UNKNOWN` reject registration (neither is
    extension-matched).
    """
    if category is AssetCategory.FOLDER or category is AssetCategory.UNKNOWN:
        raise ValueError(
