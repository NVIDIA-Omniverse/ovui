...
Action icons are resolved relative to ovwidgets.common/resources/icons/.
Prim-type and status icons are resolved relative to
ovwidgets.common/style/icons/. Placeholder SVGs ship with the package;
...
_ICONS_DIR: Path = Path(
    str(importlib.resources.files("ovwidgets.common").joinpath("resources/icons"))
)
_STYLE_ICONS_DIR: Path = Path(
    str(importlib.resources.files("ovwidgets.common.style").joinpath("icons"))
)
...
def register_urls() -> None:
    ...
    # placeholders one per category in ``ovwidgets.common.asset_types`` — the
...
# Register at import time so that importing ovwidgets.app.style is sufficient.
register_urls()
