...
(``_OPEN_DIALOGS`` in :mod:`ovwidgets.common.file_dialogs`,
:mod:`ovwidgets.common.dialogs`, and
:mod:`ovwidgets.content.widget.confirm_overwrite_dialog`).
...
:mod:`ovwidgets.stage.widget.stage_icons` to this shared module in Issue #31
Step 5 so that any widget package (e.g. ``ovwidgets.property``) can call
...
#   ``"clsmethod:ovwidgets.content.file_importer.FileImporterHelper.reset_singleton"``
...
# ---------------------------------------------------------------------------
# Elevated from ovwidgets.stage.widget.stage_icons so that any widget package
# (e.g. ovwidgets.property) can call provider() without a cross-widget import.
# The return-type annotation is a forward-reference string so omni.ui is
# NOT imported at module level — ovwidgets.common.icon_caches must remain importable
