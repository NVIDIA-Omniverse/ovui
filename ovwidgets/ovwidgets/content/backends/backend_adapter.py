...
subscribe across whatever storage the app happens to be talking to. OvGear
v1 only ships a local-filesystem backend, but every content-browser
component goes through this ABC so a future Nucleus / HTTP / S3 backend
can plug in without touching the model or widget layers.
...
class BackendFileFlags(Flag):
    ...
    convention already set by :class:`ovwidgets.common.adapters.ItemFlags` and
    :class:`ovwidgets.common.adapters.BadgeFlags` (see ``ovwidgets.app/adapters.py``).
...
class BackendAdapter(ABC):
    ...
    subscription in the content-browser stack goes through BackendAdapter.
    See the content browser behavior and the content browser implementation step 1 for the rationale.

    All synchronous: OvGear is single-threaded and defers UI refresh
    via :meth:`ovwidgets.app.application.Application.call_later`. Adapters that
    back remote storage in a future revision can run their own threads
    internally and marshal results through ``call_later``.
    """
    ...
    def subscribe_changes(
        ...
        :class:`ovwidgets.common.settings.Subscription` — it has ``cancel()``
        and ``__del__`` — so consumers can hold it without caring
        whether the backend actually subscribes.
        """
        ...
        return _NoOpSubscription()  # type: ignore[return-value]
