...
from ovwidgets.content.widget.path_field import _PATH_FIELD_HEIGHT, PathField
...
# :mod:`ovwidgets.content.widget.file_browser_delegate` so the
# two callsites share the same cache keying — a future extraction of
# the helper into ``ovwidgets.app.style`` or ``ovwidgets.app.ui_utils`` would fold
...
def _provider(path: str) -> "ui.RasterImageProvider":
    ...
    :mod:`ovwidgets.content.widget.file_browser_delegate`;
