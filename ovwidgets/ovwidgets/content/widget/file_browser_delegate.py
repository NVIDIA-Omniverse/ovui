...
# ``ovwidgets.stage.widget.stage_delegate`` to keep the expand/collapse
...
# ``ovwidgets/common/icons/``. Resolving the absolute path once at
...
# the icons are package data of ``ovwidgets-common``.
_CHEVRON_ICON_DIR = str(
    importlib.resources.files("ovwidgets.common").joinpath("icons")
)
...
# :mod:`ovwidgets.content.style`) which dims the row to
...
# ovwidgets.stage :class:`StageDelegate` rename path.
_KEY_ESCAPE = 256
...
# reliable path. Identical duplicated pattern in ovwidgets.stage.stage_icons
# and ovwidgets.property.window — see review follow-up for a shared extractor.
...
def _render_branch_chevron(item: Any, level: Any, expanded: Any) -> None:
    ...
    Mirrors :func:`ovwidgets.stage.widget.stage_delegate.StageDelegate.build_branch`
...
class FileBrowserDelegate(ui.AbstractItemDelegate):
    ...
    def _build_plugin_cell(item: FileItem, column_id: Any) -> None:
        ...
        :mod:`ovwidgets.content.widget.column_delegate` for why
