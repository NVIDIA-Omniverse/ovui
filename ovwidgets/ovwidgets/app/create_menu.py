...
"""Create-menu USD prim authoring for the standalone USD Viewer.

The mesh evaluators in this module are standalone ports of Kit's primitive
mesh evaluators. They intentionally depend only on ``pxr`` and ``omni.ui``;
there are no Kit, Carbonite, settings, or command-registry dependencies.
...
def build_create_menu(app: Any) -> None:
    ...
    ui.MenuItem("Camera", triggered_fn=lambda: create_camera(app))
    ui.Separator()
    ui.MenuItem("Scope", triggered_fn=lambda: create_scope(app))
    ui.Separator()
    ui.MenuItem("Xform", triggered_fn=lambda: create_xform(app))
...
def _show_error(message: str) -> None:
    try:
        ...
        ErrorReporter.show_warning(message)
    except Exception as exc:
        print(f"[USD Viewer] {message} (error reporter unavailable: {exc})", file=sys.stderr)
