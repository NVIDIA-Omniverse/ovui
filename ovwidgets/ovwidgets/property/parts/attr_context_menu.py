...
* **Reset to Default** — calls ``adapter.clear_value(attr_name)``.
  Adapters whose ``clear_value`` is the :class:`PropertyAdapter` ABC
  default (raises :class:`NotImplementedError`) are detected via
  :func:`can_reset` so the menu item disables instead of appearing to
...
def copy_value(
    ...
    Copy semantics in :mod:`ovwidgets.property.parts.group_context_menu`.
...
def reset_value(adapter: PropertyAdapter, attr_name: str) -> bool:
    ...
    Locked attrs are skipped for parity with the group-level
    :func:`~ovwidgets.property.parts.group_context_menu.reset_group`: the
    indicator column already signals "locked" and the USD layer would
    reject the clear anyway. Adapters whose ``clear_value`` is the ABC
    default raise :class:`NotImplementedError`; those adapters should
    be detected up-front via :func:`can_reset` so the menu item
    disables. The driver catches ``NotImplementedError`` inline in
    case a concrete adapter's ``clear_value`` raises for one specific
    attr (e.g. relationship types in the USD adapter).
...
def can_reset(adapter: PropertyAdapter, attr_name: str) -> bool:
    ...
    Disabled when the adapter has no ``clear_value`` override (the ABC
    default raises), when the target attr is locked, or when it is
    already unauthored (nothing to clear). Mirrors the ``NotDefault``
    predicate in :mod:`ovwidgets.property.parts.control_state`: the row-level
    indicator fires on "authored + adapter can clear", and the
    menu-item gate uses the same check so the two stay consistent.
    """
