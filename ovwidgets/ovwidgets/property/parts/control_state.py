...
handlers via :mod:`ovwidgets.common.style.urls`. Rendering is dual-mode: when
...
* ``NotDefault`` (priority 40) — fires on ``metadata.is_authored`` AND
  when ``adapter.clear_value`` is overridden from the ABC default (the
  USD adapter implements; the mock inherits the raising ABC default,
  which collapses this predicate to False so the icon stays hidden on
  mock-backed rows). ``on_click`` clears the authored opinion and
  reverts the row to the schema default.
...
class ControlStateHandler:
    ...
    * ``icon_path`` — filesystem path to the icon image (SVG or PNG).
      ...
      ``ovwidgets.app/style/icons/`` (Step 4.4); third-party handlers should
...
class _HandlerSubscription:
    ...
    pattern elsewhere in ovwidgets.property: callers hold the handle to keep
...
def _adapter_supports_clear(adapter: PropertyAdapter) -> bool:
    """True when ``adapter`` overrides :meth:`PropertyAdapter.clear_value`.

    The ABC default raises :class:`NotImplementedError`. Detecting the
    override via method-identity comparison is cheap (one pointer
    compare) and avoids the cost of a try/except-probe on every
    predicate evaluation. An adapter that wants to participate in the
    NotDefault state must provide its own ``clear_value`` implementation.
    """
    return type(adapter).clear_value is not PropertyAdapter.clear_value


def _not_default_predicate(model: Any) -> bool:
    """NotDefault fires when an attribute is authored *and* the adapter
    can clear it. The adapter-capability check folds the "icon hidden
    when ``on_click`` is None or not callable" rule into the predicate,
    so a mock-backed row (no ``clear_value`` override) doesn't expose a
    click handler that would raise ``NotImplementedError``.
    """
    metadata = model.metadata
    if not metadata.is_authored:
        return False
    return _adapter_supports_clear(model.adapter)
...
def _register_defaults(manager: "ControlStateManager") -> None:
    ...
    Icon paths resolve through :mod:`ovwidgets.common.style.urls` — the four
    per-state SVGs (``mixed.svg``, ``locked.svg``, ``timesample.svg``,
    ``not_default.svg``) live in ``ovwidgets.app/style/icons/`` and are
...
def _style_state_name(handler_name: str) -> str:
    ...
    style entries in ``ovwidgets.property/style.py`` stay terse. Third-party
...
class ControlStateIndicator:
    ...
    * **Standalone** (``ui._IN_KIT`` False) — the standalone omni.ui
    ...
    def _on_click(self, x: float, y: float, button: int, modifier: int) -> None:
        ...
        check in so ``on_click`` should only be reachable when
        ``adapter.clear_value`` is overridden, but a runtime swap of
