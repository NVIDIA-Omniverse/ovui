...
* Schema check pins ``{"version": 1, "windows": <dict>}`` — exactly
  what :func:`ovwidgets.app.layout.save_layout_data` writes. A regression
...
def test_layout_saved_on_clean_exit(tmp_path) -> None:
    ...
    windows, so :func:`ovwidgets.app.layout._collect_layout` still sees live
    ...
    # Parse + schema check. Must match ovwidgets.app.layout.save_layout_data:
    ...
    # from :func:`ovwidgets.app.layout._collect_layout`'s ``panel_map``. If the
