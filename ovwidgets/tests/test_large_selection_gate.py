...
Architecture: :class:`~ovwidgets.property.window.PropertyWindow._rebuild_content`
now builds a :class:`~ovwidgets.property.payload.PropertyPayload` at the top
...
at the ``ovwidgets.property.window.ui`` namespace so the banner build's
...
def patched_ui(monkeypatch):
    ...
    ``import omni.ui as ui`` in :mod:`ovwidgets.property.window`. Patching
    ...
    """
    import ovwidgets.property.window as window_mod
