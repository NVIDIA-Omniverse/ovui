...
1. **Mute Layer / Unmute Layer** — single entry with a dynamic
   ...
   :class:`~ovwidgets.layers.commands.SetLayerMutenessCommand` with the
   opposite of the current state.
2. **Lock Layer / Unlock Layer** — same shape for the lock bit,
   routed through :class:`~ovwidgets.layers.commands.SetLayerLockCommand`.
...
from ovwidgets.common.undo import UndoGroup, UndoManager
