...
- :class:`~ovwidgets.common.undo.CommandCancelled` — raising it from a command's
  :meth:`do` short-circuits :meth:`UndoManager.push` without mutating
  the undo / redo stacks.
- :class:`~ovwidgets.layers.commands.RemoveSublayerCommand` /
  :class:`~ovwidgets.layers.commands.ReloadLayerCommand` ``confirm_callback``
...
- :mod:`ovwidgets.common.dialogs` — the three helpers
...
- :class:`~ovwidgets.layers.LayerModel` ``_request_remove_sublayer`` /
...
from ovwidgets.common.undo import CommandCancelled, UndoManager
...
class TestCommandCancelled:
    def test_exported_from_undo_module(self) -> None:
        ...
        # plan prose references (``ovwidgets.common.undo.CommandCancelled``).
        from ovwidgets.common.undo import CommandCancelled as Imported
...
# ─── ovwidgets.common.dialogs — dialog helper API ────────────────────────────────────
...
class TestConfirmDialogAPI:
    def test_confirm_click_fires_on_confirm_and_closes(
        ...
        from ovwidgets.common.dialogs import _OPEN_DIALOGS, confirm_dialog
    ...
    def test_on_cancel_defaults_to_noop(self) -> None:
        pytest.importorskip("omni.ui")
        from ovwidgets.common.dialogs import confirm_dialog
...
class TestRequestRemoveSublayerCleanSkipsDialog:
    def test_clean_layer_pushes_remove_directly(
        ...
        monkeypatch.setattr(
            "ovwidgets.common.dialogs.confirm_dirty_remove_dialog", _fake_dialog,
        )
...
class TestRequestRemoveSublayerDirtyRoutesThroughDialog:
    ...
    def test_dialog_remove_without_saving_removes_dirty_as_is(
        ...
        captured: List[dict] = []
        monkeypatch.setattr(
            "ovwidgets.common.dialogs.confirm_dirty_remove_dialog",
...
class TestRequestReloadCleanSkipsDialog:
    def test_clean_layer_pushes_reload_directly(
        ...
        opened: List[Any] = []
        monkeypatch.setattr(
            "ovwidgets.common.dialogs.confirm_reload_dialog",
        ...
        assert opened == []
        assert any(isinstance(p, ReloadLayerCommand) for p in pushed)
