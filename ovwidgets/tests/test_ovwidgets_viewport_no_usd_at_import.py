...
"""CI invariant: importing ``ovwidgets.viewport`` must NOT pull in ``pxr``.

Step 26 (Rev 4 §10.5): the widget tree is OpenUSD-free at import time.
A re-export reintroducing ``OvRtxRendererAdapter`` or
``OVRTX_AVAILABLE`` into ``ovwidgets/viewport/__init__.py`` would
...
``ovwidgets.viewport`` and exits non-zero if ``pxr`` (or any USD
...
def _venv_python() -> str:
    """Return the venv python that imports ovwidgets cleanly.
...
def test_import_ovwidgets_viewport_does_not_load_pxr():
    """``import ovwidgets.viewport`` must NOT bring ``pxr`` into ``sys.modules``."""
    code = textwrap.dedent(
        ...
        if leaked:
            sys.stderr.write(
                "ovwidgets.viewport import leaked pxr submodules: "
                + ",".join(leaked)
                + "\\n"
            )
            sys.exit(1)
