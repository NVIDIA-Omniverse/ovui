...
"""Tests for ovwidgets.app._input_drain.drain_bridge_into_ui (issue #34, Step 3.3).
...
def _wheel(dx: int, dy: int) -> MouseEvent:
    return MouseEvent(
...
def test_drain_60_frames_each_with_one_move_and_one_click() -> None:
    ...
    2. ``mouse_button`` arg is ``ImGuiMouseButton_Left == 0`` for the
       ...
       in :mod:`ovwidgets.app._input_drain` puts LEFT on slot 0.
...
# Mirror of ovwidgets.app._input_bridge constants. Duplicated so the test
