...
- :meth:`LayerModel._request_save_as` opens the file dialog via
  :mod:`ovwidgets.common.file_dialogs` and, on Save, pushes a
...
class TestRequestSaveAs:
    ...
    def test_dialog_cancel_pushes_nothing(
        ...
        monkeypatch.setattr(
            "ovwidgets.common.file_dialogs.save_file_dialog", _cancelling_dialog,
        )
...
class TestDefaultFilename:
    def test_anonymous_strips_anon_prefix(
        ...
        from ovwidgets.layers.layer_model import _default_save_as_filename
...
class TestSaveFileDialogAPI:
    def test_save_path_triggers_on_selected(self) -> None:
        ...
        dialog.set_path("/tmp/chosen.usda")
        dialog.confirm()
        assert seen == ["/tmp/chosen.usda"]
