...
"""Unit tests for :mod:`ovwidgets.common.icon_caches`.
...
from ovwidgets.common import icon_caches
...
class TestProvider:
    """Tests for :func:`ovwidgets.common.icon_caches.provider`.
    ...
    def test_provider_caches_on_second_call(self, monkeypatch):
        ...
        import ovwidgets.common.icon_caches as real_ic
        created = []
    ...
    def test_provider_importable_from_ovwidgets_app_icon_caches(self):
        """Issue #31 Step 5: ``provider`` must be importable directly from
        ``ovwidgets.common.icon_caches`` (not only from the shim)."""
        from ovwidgets.common.icon_caches import provider as shared_provider
        assert callable(shared_provider)

    def test_stage_icons_provider_delegates_to_shared(self):
        """Issue #31 Step 5: ``ovwidgets.stage.widget.stage_icons.provider`` must
        be the same object as ``ovwidgets.common.icon_caches.provider`` — it is
