...
No :class:`ovwidgets.app.application.Application` singleton is built — the
...
import ovwidgets.app.application as application_module
...
class TestMockBackendSubscribeChanges:
    ...
    GC (matches :class:`ovwidgets.common.settings.Subscription`), so an
    ...
    def test_cancel_prunes_empty_url_entry(self, backend: MockBackend):
        ...
        # ``ovwidgets.common.settings.Settings._remove_subscriber``.
...
class TestBatching:
    def test_multiple_events_coalesce_when_application_is_present(
        ...
        :func:`ovwidgets.common.scheduler.call_later`. This test
...
class TestDestroyCancelsSubscription:
    ...
    def test_destroy_clears_pending_item_changed(
        ...
        Per Rev 8 §5.5 (Step 5), widget code routes through
        ``ovwidgets.common.scheduler.call_later``; this test registers
