...
1. **Save** — on-layer, gated on ``is_layer_item`` + ``is_layer_dirty``.
   ...
   :class:`~ovwidgets.layers.commands.SaveLayerCommand` for concrete layers
...
from ovwidgets.layers.commands.sublayer_commands import RemoveSublayerCommand
...
class TestSaveClick:
    ...
    def test_click_on_anonymous_layer_forwards_to_save_as(
        ...
        monkeypatch.setattr(
            "ovwidgets.common.file_dialogs.save_file_dialog", _fake_dialog
        )
...
class TestSaveAsClick:
    ...
    def test_dialog_confirm_pushes_save_as_command(
        ...
        from ovwidgets.layers.commands.file_io_commands import SaveLayerAsCommand
...
class TestReloadClick:
    ...
    def test_click_on_dirty_layer_opens_confirm_dialog(
        ...
        monkeypatch.setattr(
            "ovwidgets.common.dialogs.confirm_reload_dialog", _fake_confirm
        )
