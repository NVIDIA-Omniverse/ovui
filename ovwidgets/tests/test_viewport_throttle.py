...
* ``ovwidgets.app.frame_clock.FrameClock`` owns the cadence; the Application
...
# ---------------------------------------------------------------------------
# Constants — unchanged: viewport still publishes the foreground/background caps
# ---------------------------------------------------------------------------


class TestConstants:
    def test_foreground_fps_value(self):
        assert ViewportWidget.MAX_FPS_FOREGROUND == 60

    def test_background_fps_value(self):
        assert ViewportWidget.MAX_FPS_BACKGROUND == 10

    def test_foreground_greater_than_background(self):
        assert ViewportWidget.MAX_FPS_FOREGROUND > ViewportWidget.MAX_FPS_BACKGROUND
...
class TestLegacyOnFrameShim:
    def test_small_dt_skips_render(self):
        renderer = MagicMock()
        vp = _vp_with_fake_image(renderer, 800, 600)
        vp._on_frame(1.0 / 120)  # 120 FPS shim → below 1/60 target
        renderer.render_frame.assert_not_called()
        vp.destroy()
