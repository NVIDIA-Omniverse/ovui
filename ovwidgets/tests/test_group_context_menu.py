...
* ``can_paste`` / ``can_reset`` disable the menu items in the
  ...
  adapter's ``clear_value`` is the ABC default).
...
def _adapter_with(
    ...
    ``clear_value`` method on the instance (not the class) — used by
    the reset tests to pin exactly which attrs had their opinion
    cleared without poisoning the ABC default on every adapter in the
    suite. ``can_reset`` checks ``type(adapter).clear_value`` so we
    need to override on the type for the capability check too —
    :func:`_subclass_adapter_with_clear` handles that wrapper.
    """
    if metas is None:
...
class TestCanReset:
    ...
    def test_abc_default_disables_reset(self) -> None:
        """Adapter whose ``clear_value`` is the ABC default → disabled."""
        metas = {"x": _meta("x", is_authored=True)}
        adapter = MockPropertyAdapter(
            paths=["/World/X"], attributes=metas,
        )
        # MockPropertyAdapter inherits the ABC default — clear_value raises.
        group = _build_group({"x": 1.0}, metas=metas)
        assert can_reset(adapter, group) is False

    def test_override_and_authored_prop_enables_reset(self) -> None:
        metas = {"x": _meta("x", is_authored=True)}
        adapter = _RecordingAdapter(
            paths=["/World/X"], attributes=metas,
        )
...
class TestPropertyWindowContextMenuWiring:
    ...
    def test_on_context_menu_captures_correct_group_per_frame(
        ...
        monkeypatch.setattr(
            "ovwidgets.property.parts.group_context_menu.show_group_context_menu",
            _spy_show,
        )
