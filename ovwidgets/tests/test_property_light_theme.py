...
the property inspector step 8.3. These are the structural guardrails that
back the Step 8.3 dual-theme screenshot (``qa_full_app_step8_3_screenshot.py``).
The screenshot proves the Property Inspector renders under both shades
*at a given moment*; these tests pin the invariants that keep it
working indefinitely:

1. ``ovwidgets.property/style.py``'s ``PROPERTY_STYLES`` contains zero raw hex
...
class TestThemeSwitchPropagatesToStyleDefault:
    """:func:`ovwidgets.app.style.set_theme` re-resolves PROPERTY_STYLES on shade switch.

    The Application wires ``_on_theme_changed`` → ``set_theme`` →
    ``apply_global_styles`` (see ``ovwidgets.app/style/__init__.py`` —
