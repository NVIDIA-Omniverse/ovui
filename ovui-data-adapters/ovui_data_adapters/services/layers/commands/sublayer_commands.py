...
- :class:`CreateSublayerCommand` — mint a fresh (named or anonymous)
  ...
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.create_sublayer` on the
...
- :class:`InsertSublayerCommand` — insert an *existing* layer
  ...
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.insert_sublayer` and
...
- :class:`ReplaceSublayerCommand` — swap the identifier at
  ...
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.replace_sublayer`.
...
- :class:`RemovePrimSpecsCommand` — delete a batch of prim specs
  ...
  :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.export_prim_spec`
  *before* removing it so ``undo_impl`` can restore the exact bytes
  through :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.import_prim_spec`.
...
class CreateSublayerCommand(AbstractLayerCommand):
    ...
    :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.create_sublayer`
...
class RemoveSublayerCommand(AbstractLayerCommand):
    ...
    def __init__(
        ...
        # :class:`~ovui_widgets.common.undo.CommandCancelled`. Tests drive this
    ...
    def do_impl(self) -> None:
        ...
        # via :class:`~ovui_widgets.common.undo.CommandCancelled`, which
...
class MoveSublayerCommand(AbstractLayerCommand):
    ...
    :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.move_sublayer` owns
...
class ReplaceSublayerCommand(AbstractLayerCommand):
    ...
    :meth:`~ovui_widgets.common.adapters.LayerStackAdapter.replace_sublayer` so
