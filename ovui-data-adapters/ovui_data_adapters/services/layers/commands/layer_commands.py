...
:class:`~ovui_widgets.common.undo.UndoManager` and pick them up on ``Ctrl+Z``:
...
- :class:`SetLayerMutenessCommand` — toggle local mute on a layer.
  Driven by :class:`~ovui_widgets.layers.models.mute_model.LocalMuteValueModel`
  when the user clicks the eye-column icon.
- :class:`SetLayerLockCommand` — toggle the per-layer lock bit.
  Driven by :class:`~ovui_widgets.layers.models.lock_model.LockValueModel`
