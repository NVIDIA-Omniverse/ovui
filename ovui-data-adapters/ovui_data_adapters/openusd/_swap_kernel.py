...
* No `cupy` / `cuda-python` runtime dependency. The module talks to
  ...
  `_Cudart` thunk used elsewhere in `_livestream_tap.py`.
...
def _ensure_compiled() -> dict:
    """Return a dict with the compiled module + driver bound on this
    thread's CUDA context. Lazy + cached.
    """
    cached = _cached.get("v1")
