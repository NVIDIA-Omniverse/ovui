...
- a library of composable app widgets (`ovwidgets/` - stage browser, property inspector, content browser, viewport, composition view)
...
- Read `docs/architecture.html` (a TOC + iframe shell) and the per-section pages under `docs/architecture/section-01.html` ... `docs/architecture/section-11.html` for the architectural overview, the UI layer, the scene tools, ovwidgets, adapters, the application, and the inspector pipeline.
...
- `ovwidgets/` - app widget library and the runnable reference application (`USD Viewer`), including stage browser, property inspector, content browser, viewport, composition view, common services, and the menu/file-open wiring.
...
The runnable application lives at `ovwidgets/ovwidgets/app/application.py` and is launched as a Python module:

```
python -m ovwidgets.app
```

Headless mode (offscreen rendering) is available via `python -m ovwidgets.app.headless`.
...
### Compose a disposable ovwidgets app
...
- Composing a disposable standalone ovwidgets application (reusing the existing viewport, stage browser, property inspector, file open dialog, and selection bus, with real ovrtx rendering and screenshot-first QA) -> `skills/omniverse-ui-widgets/SKILL.md`
  - `skills/omniverse-ui-widgets/references/source-map.md` - exact imports and required calls for the existing ovwidgets classes the trial must reuse.
...
- Styling ovui or ovwidgets surfaces (palettes, colour/float/icon constants, shades, style selectors, `style_type_name_override`, widget `name` variants, `ui.style.default`, style hierarchy, the centralised style module, and styling reviews) -> `skills/omniverse-ui-styling/SKILL.md`
...
- Inspecting, screenshotting, or driving a running ovui/ovwidgets application via the `ovui-inspect` FastAPI inspector -> `skills/omniverse-ui-inspector/SKILL.md`
...
- The runnable application title is `USD Viewer`. Internal module names (`ovwidgets`, `ovui`, `ovui_data_adapters`) and Python import paths (`omni.ui`, `omni.ui_scene`) are separate from the user-visible application title.
...
- The repository is organized so a project can take the amount that matches the tool's goal: run the full application, use one or more app widgets, replace a data adapter, build directly from the Python UI layer, or let an AI coding agent generate the next tool by reusing ovwidgets through the skills above.
